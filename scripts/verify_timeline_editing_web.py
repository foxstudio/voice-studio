#!/usr/bin/env python3
"""Real editor/save round trips against an owned fixed-media service."""
...
def run():
    with tempfile.TemporaryDirectory(prefix="voice-studio-editing-web-") as temporary:
        root = Path(temporary)
        ...
        with (root / "service.log").open("w+") as log:
            try:
                ...
                browser = subprocess.Popen(
                    [shutil.which("node") or "node", str(ROOT / "scripts/verify_timeline_editing_browser.mjs")],
                ...
                if browser.wait(timeout=180):
                    raise RuntimeError("Timeline editing browser acceptance failed")
    ...
    print(json.dumps({"timeline_edit_cleanup": "complete", "provider_calls": 0}))
