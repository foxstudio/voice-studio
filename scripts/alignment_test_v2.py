...
Prerequisites:
    ...
    2. Convert models:
       uv run python -m mlx_indextts.convert_v2 --model-dir ~/Downloads/IndexTTS-2 -o ~/VoiceStudio/models/mlx-indexTTS-2.0
"""
...
# Model directory
DATA_ROOT = Path(os.environ.get("VOICE_STUDIO_DATA_DIR", "~/VoiceStudio")).expanduser()
MODELS_ROOT = Path(
    os.environ.get("VOICE_STUDIO_MODELS_DIR", str(DATA_ROOT / "models"))
).expanduser()
MODEL_DIR = Path(
    os.environ.get(
        "MLX_INDEXTTS_MODEL_DIR",
        str(MODELS_ROOT / "mlx-indexTTS-2.0"),
    )
).expanduser()
