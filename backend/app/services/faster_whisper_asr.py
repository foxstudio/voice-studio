...
DEFAULT_CT2_MODEL_ID = "mobiuslabsgmbh/faster-whisper-large-v3-turbo"
...
def model_files_available(model_path: Path) -> bool:
    return all((model_path / name).is_file() for name in REQUIRED_MODEL_FILES) and any(
        (model_path / name).is_file() for name in TOKENIZER_FILES
    )
