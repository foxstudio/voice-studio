...
from app.services import reference_audio_integrity, voice_store
...
def validate_values(*, engine_id: str, values: Mapping[str, Any], require_reference: bool = True) -> None:
    ...
    try:
        reference_audio_integrity.validate_trim_contract(
            source_duration_ms=values.get("emotion_reference_source_duration_ms"),
            trim_start_ms=values.get("emotion_reference_trim_start_ms"),
            trim_end_ms=values.get("emotion_reference_trim_end_ms"),
            code_prefix="EMOTION_REFERENCE",
            label="情绪参考音频",
        )
    except reference_audio_integrity.ReferenceAudioIntegrityError as exc:
        raise EmotionReferenceError(exc.code, exc.message) from exc
    ...
    *,
    ...
    try:
        reference_audio_integrity.validate_reference_clip(
            resolved,
            source_duration_ms=effective.get("emotion_reference_source_duration_ms"),
            trim_start_ms=effective.get("emotion_reference_trim_start_ms"),
            trim_end_ms=effective.get("emotion_reference_trim_end_ms"),
            code_prefix="EMOTION_REFERENCE",
            label="情绪参考音频",
        )
    except reference_audio_integrity.ReferenceAudioIntegrityError as exc:
