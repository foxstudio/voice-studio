...
def cleanup_orphaned_uploads(
    *,
    ...
    tasks = (
        ...
        else db.list_all("tasks", "created_at", limit=-1)
    )
    history = (
        list(history_rows)
        if history_rows is not None
        else db.list_all("history", "created_at", limit=-1)
    )
    voices = (
        list(voice_rows)
        if voice_rows is not None
        else db.list_all("voices", "updated_at", limit=-1)
    )
    deleted: list[str] = []
    candidates = [
        VoiceFile(**row)
    ...
    eligible = [
        ...
        if _is_owned_voice_file(voice_file)
        and _older_than_ttl(
            voice_file.created_at,
            current,
            ttl_seconds,
        )
    ]
    ...
    candidate_ids = {voice_file.file_id for voice_file in eligible}
    candidate_paths = {
        _resolved(voice_file.path): voice_file.file_id
        for voice_file in eligible
    }
    referenced: set[str] = set()
    for row in tasks:
        _collect_candidate_references(
            row.get("parameters", {}),
            candidate_ids,
            candidate_paths,
            referenced,
    ...
    for table, order_field in (
        ...
        for row in db.list_all(table, order_field, limit=-1):
            _collect_candidate_references(
                row,
                candidate_ids,
                candidate_paths,
    ...
    for row in voices:
        referenced.update(
            candidate_ids.intersection(
                str(value)
                for value in row.get("reference_audio_ids", [])
            )
        )

    for voice_file in eligible:
        if voice_file.file_id in referenced:
            continue
        current_voice_file = _get_voice_file(voice_file.file_id)
        if (
            current_voice_file is None
            or not _is_owned_voice_file(current_voice_file)
            or _resolved(current_voice_file.path)
            != _resolved(voice_file.path)
        ):
            continue
        ...
        db.delete_one(
            "voice_files",
            "file_id",
            current_voice_file.file_id,
        )
        deleted.append(current_voice_file.file_id)
    return deleted
...
def _collect_candidate_references(
    ...
    if isinstance(value, dict):
        ...
        if value in candidate_ids:
            output.add(value)
            return
        if not is_managed_custom_path(value):
            return
        file_id = candidate_paths.get(_resolved(value))
