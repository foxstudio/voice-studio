...
from app.services.python_runtime import virtualenv_python
...
_worker_identity: tuple[str, str, str] | None = None
...
def runtime_python() -> Path | None:
    explicit = os.environ.get("VOICE_STUDIO_QWEN_ALIGN_PYTHON")
    settings = settings_store.get()
    candidates = [
        _non_resolving_path(explicit) if explicit else None,
        virtualenv_python(
            expand_path(settings.data_dir) / "engines" / "qwen-forced-aligner" / ".venv"
        ),
        virtualenv_python(PROJECT_ROOT / ".venv-qwen-align"),
    ]
...
def checkpoint_candidates() -> list[Path]:
    base = settings_store.managed_model_root()
    candidates = [
        *settings_store.model_candidates("qwen3-forced-aligner"),
...
def health_check() -> dict[str, Any]:
    ...
    if not python_path:
        return {
            ...
            "detail": "未找到 Qwen 对齐运行环境；请安装到受管引擎目录或设置 VOICE_STUDIO_QWEN_ALIGN_PYTHON",
...
def _checkpoint_is_complete(path: Path) -> bool:
    if not checkpoint_files_available(path):
        return False
...
def checkpoint_files_available(path: Path) -> bool:
    if not path.is_dir() or any(
        not (path / name).is_file() for name in REQUIRED_CHECKPOINT_FILES
    ):
        return False
    if any(path.rglob("*.incomplete")):
        return False
    weights = path / "model.safetensors"
    return weights.is_file() and weights.stat().st_size >= MIN_CHECKPOINT_BYTES
...
def _ensure_worker() -> subprocess.Popen[str]:
    global _worker, _worker_identity, _worker_log_handle
    ...
    with _worker_lock:
        ...
        identity = (str(python_path), str(checkpoint.resolve()), device)
        if _worker and _worker.poll() is None:
            if _worker_identity == identity:
                return _worker
            _reset_worker()
