...
PROVIDERS = ProviderRegistry(
    (
        *builtin_providers(),
        *vibevoice_asr.providers(),
        speaker_diarization_service.provider(),
    )
)
...
def transcribe(
    *,
    ...
    return {
        ...
        "incomplete_chunk_ranges": result.metadata.get("incomplete_chunk_ranges") or [],
    }


def supports_joint_analysis(engine_id: str) -> bool:
    provider = PROVIDERS.get(engine_id)
    return bool(
        provider
        and provider.capabilities.supports_transcription
        and provider.capabilities.supports_diarization
        and callable(getattr(provider, "transcribe_and_diarize", None))
    )
...
def transcribe_and_diarize(
    *,
    ...
    ensure_engine_ready(engine_id)
    provider = PROVIDERS.get(engine_id)
    joint = getattr(provider, "transcribe_and_diarize", None) if provider is not None else None
    if not callable(joint):
        raise AppException(
            400,
            "ASR_JOINT_ANALYSIS_UNSUPPORTED",
            f"这个 ASR 引擎不能在一次推理中同时返回文字和说话人：{engine_id}",
        )
    asr_result, diarization_result = joint(
        audio_path,
        language=language,
        hotwords=hotwords,
    ...
    return (
        {
            ...
            "usage_seconds": asr_result.metadata.get("usage_seconds"),
            "provider_response_id": asr_result.metadata.get("provider_response_id"),
            "incomplete_chunk_ranges": asr_result.metadata.get("incomplete_chunk_ranges") or [],
            "metadata": dict(asr_result.metadata),
        },
        {
            "engine_id": diarization_result.provider_id,
            "model_id": str(diarization_result.metadata.get("variant_id") or "") or None,
            "segments": [
                {
                    "start_ms": item.start_ms,
                    "end_ms": item.end_ms,
                    "speaker": item.speaker_cluster,
                    "source_speaker": item.speaker_cluster,
                    "confidence": item.confidence,
                }
                for item in diarization_result.segments
            ],
            "metadata": dict(diarization_result.metadata),
        },
    )


def _unavailable_codes_for(engine_id: str) -> dict[str, str]:
    if engine_id.startswith("vibevoice-asr-mlx-"):
        runtime_missing = "VIBEVOICE_ASR_RUNTIME_MISSING"
        model_missing = "VIBEVOICE_ASR_MODEL_MISSING"
    elif engine_id == "qwen3-asr-mlx":
