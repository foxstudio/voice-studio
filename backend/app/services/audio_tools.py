...
def probe_audio_duration_ceil_ms(path: str | Path) -> int:
    """Return the smallest whole millisecond that contains every sample."""

    info = sf.info(str(path))
    if not info.samplerate:
        return 0
    return math.ceil(info.frames * 1_000 / info.samplerate)
...
def read_audio_multichannel(path: str | Path) -> tuple[np.ndarray, int]:
    """Read an audio file without collapsing its channel layout."""
