...
def get() -> WebSearchSettings:
    ...
    if raw:
        try:
            ...
            pass
    # Older settings stored a global query budget. Keep those rows readable,
    # but query-count policy now belongs to each versioned workflow.
    payload.pop("max_queries", None)
