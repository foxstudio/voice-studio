...
def create_audio_clip(file_id: str, start_ms: int, end_ms: int, *, clip_file_id: str | None = None) -> dict:
    ...
    if clip_file_id:
        ...
        if existing and existing.path and Path(existing.path).exists():
            ...
            try:
                actual_duration_ms = int(audio_tools.probe_audio(existing.path).get("duration_ms") or 0)
            except Exception:
                actual_duration_ms = 0
            tolerance_ms = max(150, round(expected_duration_ms * 0.03))
            if actual_duration_ms > 0 and abs(actual_duration_ms - expected_duration_ms) <= tolerance_ms:
                if existing.duration_ms != actual_duration_ms:
                    existing.duration_ms = actual_duration_ms
                    db.upsert("voice_files", existing.file_id, existing.model_dump())
                quality = _quality_for_voice_file(actual_duration_ms)
