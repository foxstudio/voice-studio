...
BUILTIN_PRESETS: list[PresetTemplate] = [
    ...
    PresetTemplate(
        ...
        name="Qwen3 Vivian 口播",
        scene="Qwen3-TTS MLX / 预置音色",
        description="使用 Qwen3 CustomVoice 的 Vivian 预置音色，适合先做中文短句试听。",
        engine_id="qwen3-tts-mlx-0.6b",
        sample_text="大家好，这是 Qwen3-TTS 本地 MLX 引擎的中文短句试听。",
        parameters={"language": "zh", "output_format": "wav", "speaker_id": "Vivian", "style_instruction": "自然、清晰、语速适中，不要夸张。", "temperature": 0.7},
    ...
    PresetTemplate(
        ...
        description="使用本地音色库参考音频走 Qwen3 Base 复刻；适合短句测试授权参考音色。",
        engine_id="qwen3-tts-mlx-0.6b",
        sample_text="接下来，我们用这个参考音色生成一段自然、稳定的讲述测试。",
        parameters={"language": "zh", "output_format": "wav", "style_instruction": "保持参考音色，语速适中，停顿自然。", "temperature": 0.7},
