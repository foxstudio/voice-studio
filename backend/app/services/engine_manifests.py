    ...
    "doubao-tts-preset": EngineDetail(
        manifest=EngineManifest(
            ...
            parameter_schema=[
                ...
                ParameterSchema(key="loudness_rate", label="音量", type="slider", default=0, min=-50, max=100, step=5, level="advanced", description="官方 loudness_rate，-50 为 0.5 倍，0 为原始音量，100 为 2 倍"),
                ParameterSchema(key="pitch_rate", label="音调", type="slider", default=0, min=-12, max=12, step=1, level="advanced", description="映射到官方 additions.post_process.pitch，0 为原始音调"),
                ParameterSchema(key="sample_rate", label="采样率", type="select", default=doubao_client.DEFAULT_TTS_SAMPLE_RATE, level="advanced", options=[{"label": f"{rate / 1000:g} kHz", "value": rate} for rate in [8000, 16000, 22050, 24000, 32000, 44100, 48000]], description="官方 sample_rate，可选 8–48 kHz"),
                ParameterSchema(key="bit_rate", label="MP3 码率", type="select", default=doubao_client.DEFAULT_TTS_BIT_RATE, level="advanced", options=[{"label": f"{rate // 1000} kbps", "value": rate} for rate in [64000, 96000, 128000, 160000]], description="官方 bit_rate，仅在 MP3 输出时生效，范围 64–160 kbps"),
                ParameterSchema(key="enable_subtitle", label="返回字级时间戳", type="toggle", default=False, level="advanced", description="官方 enable_subtitle，仅 TTS 2.0 的中文和英文支持"),
                ParameterSchema(key="silence_duration", label="结尾静音", type="number", default=0, min=0, max=30000, step=100, level="advanced", description="官方 additions.silence_duration，单位毫秒，范围 0–30000"),
                ParameterSchema(key="aigc_watermark", label="AIGC 声音标识", type="toggle", default=False, level="advanced", description="官方 aigc_watermark，开启后在合成音频结尾加入节奏标识"),
                ParameterSchema(
                    key="style_instruction",
                    label="语音指令",
                    type="textarea",
        ...
        manifest=EngineManifest(
            ...
            parameter_schema=[
                ParameterSchema(
                    ...
                    description="Seed Audio 官方支持的输出音频格式。",
                ),
                ParameterSchema(
                    key="sample_rate",
                    label="采样率",
                ...
                ParameterSchema(key="enable_subtitle", label="返回字幕时间戳", type="toggle", default=False, level="advanced"),
                ParameterSchema(key="speech_rate", label="整体语速", type="slider", default=0, min=-50, max=100, step=1),
                ParameterSchema(key="loudness_rate", label="整体音量", type="slider", default=0, min=-50, max=100, step=1, level="advanced"),
                ParameterSchema(key="pitch_rate", label="整体音调", type="slider", default=0, min=-12, max=12, step=1, level="advanced"),
                ParameterSchema(key="aigc_watermark", label="显式音频水印", type="toggle", default=False, level="advanced"),
                ParameterSchema(key="aigc_metadata_enable", label="隐式元数据水印", type="toggle", default=False, level="advanced"),
                ParameterSchema(key="content_producer", label="内容制作方", type="text", default="", level="advanced"),
                ParameterSchema(key="produce_id", label="内容制作编号", type="text", default="", level="advanced"),
                ParameterSchema(key="content_propagator", label="内容传播方", type="text", default="", level="advanced"),
                ParameterSchema(key="propagate_id", label="内容传播编号", type="text", default="", level="advanced"),
            ],
        ),
