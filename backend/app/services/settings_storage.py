...
def audit(settings: AppSettings, database_path: Path) -> dict[str, Any]:
    ...
    locations = [
        ...
        _location(
            ...
            cleanup_key="waveforms",
            cleanup_label="清理波形缓存",
            cleanup_risk="low",
        ),
        _location(
            "qwen_align",
        ...
        _location(
            "video_preview",
            "视频预览缓存",
            paths["cache_dir"] / "video-preview",
            category="可重建缓存",
            description="视频本土化时间线的分块预览帧；按空间上限自动回收，可随时重建。",
            cleanup_key="video_preview",
            cleanup_label="清理预览缓存",
            cleanup_risk="low",
        ),
        _project_preview_proxy_location(paths["project_dir"]),
        _location(
            "log_dir",
            "应用日志",
            paths["log_dir"],
            category="日志",
            description=(
                "应用层日志目录；常驻模型 worker 日志单文件上限 5 MB 并保留 2 份轮转。"
                "部分外部引擎仍在自身根目录写受同样上限保护的 .voice_studio 日志。"
            ),
            cleanup_key="logs",
...
def _paths(settings: AppSettings) -> dict[str, Path]:
    ...
    return {
        ...
        "model_dir": model_store.managed_model_root(settings),
...
def _flows(paths: dict[str, Path]) -> list[dict[str, str]]:
    return [
        ...
        {
            "name": "视频时间线预览",
            "path": str(paths["cache_dir"] / "video-preview" / "<project_id>" / "<source-signature>"),
            "description": "按时间块保存轻量预览帧；播放代理仍保留在项目目录，二者互不替代。",
        },
    ]
...
def _project_preview_proxy_location(project_dir: Path) -> dict[str, Any]:
    from app.domains.video_localization import media_assets

    stats = media_assets.preview_proxy_storage_stats(project_dir)
    return {
        "key": "video_media_proxy",
        "label": "视频与音轨播放代理",
        "path": str(project_dir),
        "category": "可重建缓存",
        "description": "为浏览器流畅播放生成的轻量视频和音频副本；源视频、分轨和配音结果不会被清理。",
        "cleanup_key": "video_media_proxy",
        "cleanup_label": "清理播放代理",
        "cleanup_risk": "low",
