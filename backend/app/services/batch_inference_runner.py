...
def _cloud_failure(
    *,
    ...
    definite_response = bool(
        getattr(error, "status_code", None) is not None
    ...
    return {
        ...
        "provider_log_id": getattr(error, "logid", None),
        "provider_state_uncertain": not definite_response,
    }
...
def run_omnivoice(payload: dict[str, Any]) -> list[dict[str, Any]]:
    import numpy as np
    import soundfile as sf
    from app.services.inference_runner import load_omnivoice_model, omnivoice_local_snapshot
    from omnivoice.models.omnivoice import OmniVoiceGenerationConfig

    common = dict(payload["common"])
    model_dir = common.pop("model_dir", None)
    model = load_omnivoice_model(
        omnivoice_local_snapshot(model_dir),
        common.pop("device", "mps"),
    )
    results = []
    for segment in payload["segments"]:
        started = time.perf_counter()
    ...
    for segment in payload["segments"]:
        ...
        try:
            result = doubao_client.generate_tts_unidirectional_http(
                ...
                request_id=kwargs.get("provider_request_id"),
            )
            if provider_out != out:
                from app.services import audio_tools
