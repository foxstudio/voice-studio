...
def data_root(environ: Mapping[str, str] | None = None) -> Path:
    ...
    return Path(env.get("VOICE_STUDIO_DATA_DIR", "~/VoiceStudio")).expanduser()


def model_candidates(
    settings_base: Path | None = None,
    *,
    environ: Mapping[str, str] | None = None,
) -> list[Path]:
    env = os.environ if environ is None else environ
    candidates: list[Path] = []
    env_value = env.get(MODEL_ENV)
    if env_value:
        candidates.append(Path(env_value).expanduser())
    if settings_base is not None:
        candidates.append(settings_base / ENGINE_ID)
    candidates.append(data_root(env) / "models" / ENGINE_ID)
...
def runtime_root(
    configured_data_root: Path | None = None,
    *,
    environ: Mapping[str, str] | None = None,
) -> Path:
    env = os.environ if environ is None else environ
    env_value = env.get(RUNTIME_ENV)
    if env_value:
        return Path(env_value).expanduser()
    root = configured_data_root or data_root(env)
    return root.expanduser() / "engines" / "mlx-audio-confucius4"
