...
def _verified_omnivoice_directory(path: Path) -> tuple[Path | None, list[str]]:
    missing = [name for name in OMNIVOICE_REQUIRED_FILES if not (path / name).is_file()]
    return (path.resolve(), []) if not missing else (None, missing)


def omnivoice_local_snapshot(preferred_model_dir: str | Path | None = None) -> Path:
    incomplete: list[tuple[Path, list[str]]] = []
    if preferred_model_dir:
        preferred = Path(preferred_model_dir).expanduser()
        if preferred.exists():
            verified, missing = _verified_omnivoice_directory(preferred)
            if verified is not None:
                return verified
            incomplete.append((preferred, missing))
    ...
    for snapshot in candidates:
        verified, missing = _verified_omnivoice_directory(snapshot)
        if verified is not None:
            return verified
...
def run_indextts_v2(**kwargs):
    ...
    cache_key = f"{engine_id}:{Path(model_dir).expanduser().resolve()}:{device}"
    with _model_cache_lock:
...
def load_omnivoice_model(local_model_path: str | Path, device: str):
    ...
    return OmniVoice.from_pretrained(str(local_model_path), **load_kwargs)
...
def run_omnivoice(**kwargs):
    ...
    start = time.perf_counter()
    local_model_path = omnivoice_local_snapshot(model_dir)
    cache_key = f"{engine_id}:{local_model_path}:{device}"
    with _model_cache_lock:
        model = _model_cache.get(cache_key)
        if model is None:
            model = load_omnivoice_model(local_model_path, device)
            _model_cache[cache_key] = model
...
def run_emotivoice(**kwargs):
    ...
    start = time.perf_counter()
    with exclusive_file_lock(root / ".voice_studio" / "emotivoice.lock"), tempfile.TemporaryDirectory(prefix="voice-studio-emotivoice-") as tmp:
        tmp_dir = Path(tmp)
...
def run_confucius4_mlx(**kwargs):
    ...
    start = time.perf_counter()
    with exclusive_file_lock(model_dir / ".voice_studio" / "confucius4-mlx.lock"), tempfile.TemporaryDirectory(prefix="voice-studio-confucius4-") as tmp:
...
def _build_qwen3_tts_kwargs(**kwargs):
    ...
    raw_speed = kwargs.pop("speed", 1.0)
    raw_temperature = kwargs.pop("temperature", qwen3_tts_paths.DEFAULT_TEMPERATURE)
    raw_top_p = kwargs.pop("top_p", qwen3_tts_paths.DEFAULT_TOP_P)
    raw_top_k = kwargs.pop("top_k", qwen3_tts_paths.DEFAULT_TOP_K)
    raw_repetition_penalty = kwargs.pop(
        "repetition_penalty",
        qwen3_tts_paths.DEFAULT_REPETITION_PENALTY,
    )
    raw_max_tokens = kwargs.pop("max_tokens", 1200)
    speed = 1.0 if raw_speed is None else float(raw_speed)
    temperature = (
        qwen3_tts_paths.DEFAULT_TEMPERATURE
        if raw_temperature is None
        else float(raw_temperature)
    )
    top_p = qwen3_tts_paths.DEFAULT_TOP_P if raw_top_p is None else float(raw_top_p)
    top_k = qwen3_tts_paths.DEFAULT_TOP_K if raw_top_k is None else int(raw_top_k)
    repetition_penalty = (
        qwen3_tts_paths.DEFAULT_REPETITION_PENALTY
        if raw_repetition_penalty is None
        else float(raw_repetition_penalty)
    )
    max_tokens = 1200 if raw_max_tokens is None else int(raw_max_tokens)
...
def _prepare_qwen3_tts_request(
    ...
    *,
    ...
    root, output_path, text, ref_audio, ref_text, language, speaker_id, instruction, voice_design_prompt, speed, temperature, top_p, top_k, repetition_penalty, max_tokens = _build_qwen3_tts_kwargs(**dict(kwargs))
    root = runtime_root or _external_root(qwen3_tts_paths.ENGINE_ID)
    if not text:
        raise RuntimeError("Text is empty")
    model_kind = "base" if ref_audio else "design" if voice_design_prompt else "custom"
    model_dir = qwen3_tts_paths.model_dir(model_kind, runtime_root=root)
...
def _finalize_qwen3_tts_output(payload: dict, *, started: float) -> dict:
    output_path = payload["output_path"]
    speed = float(payload["speed"])
    # The current MLX Qwen3 runtime accepts ``speed`` but does not yet apply it.
    # Keep passing the field upstream for forward compatibility and guarantee
    # the visible duration change with pitch-preserving post-processing.
...
def run_qwen3_tts(**kwargs):
    ...
    python = _external_python(root)
    payload = _prepare_qwen3_tts_request(kwargs, runtime_root=root)
    start = time.perf_counter()
    with exclusive_file_lock(root / ".voice_studio" / "qwen3-tts.lock"), tempfile.TemporaryDirectory(prefix="voice-studio-qwen3-payload-") as tmp:
        ...
        _run_external(
            [
                python,
                "-m",
                "app.services.qwen3_tts_runtime",
                "--once",
                str(payload_path),
            ],
            root,
    ...
    return _finalize_qwen3_tts_output(payload, started=start)
...
def run_f5_tts(**kwargs):
    device = str(kwargs.get("device") or "mps")
...
model = F5TTS(device=payload["device"], ckpt_file=payload["ckpt"], vocab_file=payload["vocab"])
model.infer(
