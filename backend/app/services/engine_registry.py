...
from app.services import cosyvoice_worker, doubao_speaker_catalog_store, engine_health, engine_manifests, engine_provider, engine_runner, f5_worker, qwen3_tts_worker
...
def start_engine(engine_id: str) -> EngineDetail:
    ...
    with _engine_state_lock:
        if not hc.get("healthy"):
            ...
            detail.state.error_message = engine_health.startup_failure_message(hc)
            return engine_provider.get_engine_detail(engine_id) or detail
