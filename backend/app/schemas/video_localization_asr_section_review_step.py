"""Durable Provider-call contracts for managed ASR section review."""

from __future__ import annotations

import hashlib
import json
import re
from typing import Literal, TypeVar

from pydantic import (
    BaseModel,
    ConfigDict,
    Field,
    field_validator,
    model_validator,
)

from app.schemas.video_localization_llm_observability import (
    VideoLocalizationLlmCallRecord,
)


ASR_SECTION_REVIEW_DEVELOPMENT_WORKFLOW_VERSION = (
    "asr-section-review-development-workflow-v1"
)
ASR_SECTION_REVIEW_CALL_INPUT_SCHEMA_VERSION = (
    "asr-section-review-call-input-v1"
)
ASR_SECTION_REVIEW_CALL_ARTIFACT_SCHEMA_VERSION = (
    "asr-section-review-call-artifact-v1"
)
_SHA256 = re.compile(r"^[0-9a-f]{64}$")
ModelT = TypeVar("ModelT", bound=BaseModel)


class AsrSectionReviewCallInputV1(BaseModel):
    """Secret-free identity of one section-review model attempt."""

    model_config = ConfigDict(extra="forbid", frozen=True)

    schema_version: Literal[
        "asr-section-review-call-input-v1"
    ] = ASR_SECTION_REVIEW_CALL_INPUT_SCHEMA_VERSION
    behavior_version: Literal[
        "asr-section-review-v5"
    ] = "asr-section-review-v5"
    behavior_fingerprint: str
    call_id: str = Field(min_length=1, max_length=256)
    section_id: str = Field(min_length=1, max_length=256)
    section_start_ordinal: int = Field(ge=1)
    core_segment_ids: tuple[str, ...] = Field(min_length=1)
    attempt: int = Field(ge=1, le=2)
    round_index: Literal[1] = 1
    system_prompt_fingerprint: str
    user_payload_fingerprint: str
    profile_id: str = Field(min_length=1, max_length=128)
    model_id: str = Field(min_length=1, max_length=256)
    provider_protocol: str = Field(min_length=1, max_length=64)
    provider_endpoint_fingerprint: str
    profile_configuration_fingerprint: str
    max_tokens: int = Field(ge=1, le=64_000)
    timeout_ms: int = Field(ge=1, le=600_000)
    disable_reasoning: bool

    @field_validator(
        "behavior_fingerprint",
        "system_prompt_fingerprint",
        "user_payload_fingerprint",
        "provider_endpoint_fingerprint",
        "profile_configuration_fingerprint",
    )
    @classmethod
    def validate_fingerprint(cls, value: str) -> str:
        return _fingerprint(value, "section-review call")

    @field_validator("core_segment_ids")
    @classmethod
    def validate_core_segment_ids(
        cls,
        value: tuple[str, ...],
    ) -> tuple[str, ...]:
        if len(set(value)) != len(value):
            raise ValueError(
                "section-review core segment IDs must be unique"
            )
        return value


class AsrSectionReviewCallArtifactV1(BaseModel):
    """Normalized model response and observable call record."""

    model_config = ConfigDict(extra="forbid", frozen=True)

    schema_version: Literal[
        "asr-section-review-call-artifact-v1"
    ] = ASR_SECTION_REVIEW_CALL_ARTIFACT_SCHEMA_VERSION
    call_input_fingerprint: str
    call_id: str = Field(min_length=1, max_length=256)
    section_id: str = Field(min_length=1, max_length=256)
    section_start_ordinal: int = Field(ge=1)
    core_segment_ids: tuple[str, ...] = Field(min_length=1)
    attempt: int = Field(ge=1, le=2)
    response: dict
    llm_call: VideoLocalizationLlmCallRecord

    @field_validator("call_input_fingerprint")
    @classmethod
    def validate_fingerprint(cls, value: str) -> str:
        return _fingerprint(value, "section-review artifact")

    @model_validator(mode="after")
    def validate_call_identity(
        self,
    ) -> "AsrSectionReviewCallArtifactV1":
        if (
            self.llm_call.call_id
            != f"{self.call_id}-a{self.attempt:02d}"
            or self.llm_call.purpose != "section_review"
            or self.llm_call.round_index != 1
            or tuple(self.llm_call.candidate_ids)
            != self.core_segment_ids
        ):
            raise ValueError(
                "section-review call record identity mismatch"
            )
        return self


def section_review_call_input(
    *,
    behavior_version: str,
    behavior_fingerprint: str,
    call_id: str,
    section_id: str,
    section_start_ordinal: int,
    core_segment_ids: tuple[str, ...],
    attempt: int,
    round_index: int,
    system_prompt: str,
    user_payload: dict,
    profile_id: str,
    model_id: str,
    provider_protocol: str,
    provider_endpoint_fingerprint: str,
    profile_configuration_fingerprint: str,
    max_tokens: int,
    timeout: float,
    disable_reasoning: bool,
) -> AsrSectionReviewCallInputV1:
    return AsrSectionReviewCallInputV1(
        behavior_version=behavior_version,
        behavior_fingerprint=behavior_fingerprint,
        call_id=call_id,
        section_id=section_id,
        section_start_ordinal=section_start_ordinal,
        core_segment_ids=core_segment_ids,
        attempt=attempt,
        round_index=round_index,
        system_prompt_fingerprint=_hash_text(system_prompt),
        user_payload_fingerprint=_hash_json(user_payload),
        profile_id=profile_id,
        model_id=model_id,
        provider_protocol=provider_protocol,
        provider_endpoint_fingerprint=provider_endpoint_fingerprint,
        profile_configuration_fingerprint=(
            profile_configuration_fingerprint
        ),
        max_tokens=max_tokens,
        timeout_ms=max(1, int(round(timeout * 1_000))),
        disable_reasoning=disable_reasoning,
    )


def section_review_call_input_fingerprint(
    value: AsrSectionReviewCallInputV1,
) -> str:
    return hashlib.sha256(_canonical_bytes(value)).hexdigest()


def section_review_call_artifact_bytes(
    value: AsrSectionReviewCallArtifactV1,
) -> bytes:
    return _canonical_bytes(value)


def parse_section_review_call_artifact(
    content: bytes,
) -> AsrSectionReviewCallArtifactV1:
    return _parse_model(
        content,
        AsrSectionReviewCallArtifactV1,
        label="section-review call artifact",
    )


def _fingerprint(value: str, label: str) -> str:
    normalized = str(value).strip()
    if _SHA256.fullmatch(normalized) is None:
        raise ValueError(
            f"{label} fingerprint must be lowercase SHA-256"
        )
    return normalized


def _hash_text(value: str) -> str:
    return hashlib.sha256(value.encode("utf-8")).hexdigest()


def _hash_json(value: object) -> str:
    return hashlib.sha256(
        json.dumps(
            value,
            ensure_ascii=False,
            sort_keys=True,
            separators=(",", ":"),
            allow_nan=False,
        ).encode("utf-8")
    ).hexdigest()


def _canonical_bytes(value: BaseModel) -> bytes:
    return (
        json.dumps(
            value.model_dump(mode="json"),
            ensure_ascii=False,
            sort_keys=True,
            separators=(",", ":"),
            allow_nan=False,
        )
        + "\n"
    ).encode("utf-8")


def _parse_model(
    content: bytes,
    model: type[ModelT],
    *,
    label: str,
) -> ModelT:
    try:
        payload = json.loads(content.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ValueError(f"{label} is not valid JSON") from exc
    return model.model_validate(payload)


__all__ = [
    "ASR_SECTION_REVIEW_CALL_ARTIFACT_SCHEMA_VERSION",
    "ASR_SECTION_REVIEW_CALL_INPUT_SCHEMA_VERSION",
    "ASR_SECTION_REVIEW_DEVELOPMENT_WORKFLOW_VERSION",
    "AsrSectionReviewCallArtifactV1",
    "AsrSectionReviewCallInputV1",
    "parse_section_review_call_artifact",
    "section_review_call_artifact_bytes",
    "section_review_call_input",
    "section_review_call_input_fingerprint",
]
