...
from app.services import asr_service, audio_tools, database as db, doubao_client, settings_store, voice_store, waveform_cache
...
@router.get("/files/{file_id}/waveform", response_model=WaveformPeaksResponse)
async def get_voice_file_waveform(
    file_id: str,
    bins: int = Query(
        default=2_400,
        ge=waveform_cache.MIN_BINS,
        le=waveform_cache.MAX_BINS,
    ),
):
    vf = voice_store.get_file(file_id)
    if not vf or not vf.path or not Path(vf.path).exists():
    ...
    return await asyncio.to_thread(
        ...
        Path(vf.path),
        result_id=f"voice-file-{file_id}",
        bins=bins,
