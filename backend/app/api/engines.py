...
async def list_engine_installations():
    return await asyncio.to_thread(model_catalog.list_installations)


@router.get("/asr-selection")
async def get_asr_selection():
    selection = await asyncio.to_thread(
        asr_selection_policy.select,
    ...
    return {
        ...
        "diarization_engine_id": selection.diarization_engine_id,
        "reason": selection.reason,
        "physical_memory_bytes": vibevoice_model.physical_memory_bytes(),
        "policy": {
            "short_or_single_speaker": "qwen3-asr-mlx",
            "long_multi_speaker_preference": [
                "vibevoice-asr-mlx-4bit",
                "vibevoice-asr-mlx-8bit",
                "qwen3-asr-mlx + moss-transcribe-diarize-mlx",
            ],
            "official_full_model_auto_selected": False,
        },
    }


@router.post("/installations/{engine_id}/install", status_code=202)
async def install_managed_model(
    engine_id: str,
    data: ManagedModelInstallRequest | None = None,
):
    if engine_id not in {
        stem_separation_model.ENGINE_ID,
        omnivoice_model.ENGINE_ID,
        *vibevoice_model.INSTALLATION_TO_VARIANT,
    }:
        raise AppException(
            400,
    ...
    if not model_catalog.automatic_download_allowed(engine_id):
        blockers = model_catalog.automatic_download_blockers(engine_id)
        license_blocked = any(
            blocker in {"model_license_unverified", "model_license_missing"}
        ...
        raise AppException(
            ...
            (
                ...
                if license_blocked
                else "MODEL_AUTOMATIC_INSTALL_BLOCKED"
            ),
            (
                "模型权重许可尚未核验，不能自动下载；请查看官方来源。"
                if license_blocked
                else "此模型的自动安装条件不完整；请查看官方来源和阻断原因。"
            ),
            {"blockers": blockers},
        )
    if engine_id == stem_separation_model.ENGINE_ID:
        return await asyncio.to_thread(stem_separation_model.start_install)
    if engine_id in vibevoice_model.INSTALLATION_TO_VARIANT:
        return await asyncio.to_thread(vibevoice_model.start_install, engine_id)
    accepted_license_id = data.accepted_license_id if data is not None else None
    return await asyncio.to_thread(
        omnivoice_model.start_install,
        accepted_license_id,
    )


@router.delete("/installations/{engine_id}")
async def uninstall_managed_model(engine_id: str):
    if engine_id != stem_separation_model.ENGINE_ID:
        raise AppException(
            400,
            "ENGINE_AUTOMATIC_UNINSTALL_UNSUPPORTED",
            "这个引擎暂不支持自动删除",
        )
    return await asyncio.to_thread(stem_separation_model.uninstall)
    ...
    try:
        ...
        if engine_id == "indextts-v2":
            ...
            kwargs = {
                "text": data.text,
                "output_path": str(output_path),
                "base_url": settings.mimo_base_url or "https://api.xiaomimimo.com/v1",
                "api_key": api_key,
                "model": model,
                "voice": "mimo_default",
                "instruction": None,
