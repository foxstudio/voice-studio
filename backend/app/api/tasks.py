...
def list_tasks_page(
    ...
    return TaskPageResponse(
        ...
        limit=limit,
        summary=TaskSummaryResponse(
            **task_queue.task_summary(reconcile_active=False)
        ),
...
async def websocket(ws: WebSocket):
    await ws.accept()
    await asyncio.to_thread(task_queue.add_ws_client, ws)
    try:
        while True:
            await ws.receive_text()
    except WebSocketDisconnect:
        await asyncio.to_thread(task_queue.remove_ws_client, ws)
