"""Authoritative operation detail reader for managed whole recheck."""
...
from app.domains.video_localization import (
    asr_whole_recheck_operation_projection as projection,
...
def assemble_whole_recheck_detail(
...
def _result_summary(
    ...
    *,
    ...
    summary["stage"] = {
        "failed": "第 1 轮全文复核失败",
        "cancelled": "第 1 轮全文复核已取消",
    }.get(status, summary["stage"])
    step_result = summary["task_step_results"]["whole_recheck_r1"]
    latest = managed_local_detail.latest_step(
        steps,
        lambda step: step.step_id == "finalize_whole_recheck",
    )
    if status == "failed":
        ...
        step_result["status"] = (
            managed_local_detail.normalized_step_status(latest.status)
        )
    managed_local_detail.apply_workflow_duration(
        summary,
        steps,
...
def _progress(status: str, steps) -> float:
    ...
    if any(
        step.status == "success"
        and step.step_id == "prepare_whole_recheck_input"
        for step in steps
    ):
        return 0.72
    return 0.60
...
def _error_code(status: str, steps, attempts) -> str | None:
    ...
    return "VIDEO_LOCALIZATION_WHOLE_RECHECK_FAILED"
...
def _error_message(
    ...
    if error_code and "RESULT_UNKNOWN" in error_code:
        return "全文复核请求的结果暂时无法确认，系统没有自动重复请求。"
    return "全文复核未完成，请检查模型配置后重试。"


__all__ = ["assemble_whole_recheck_detail"]
