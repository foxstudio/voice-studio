...
def with_updated_localized_subtitle(
    ...
    if {"start_ms", "end_ms"}.intersection(patch.model_fields_set):
        _assert_localized_subtitle_track_valid(
            next_subtitles,
            focus_subtitle_id=subtitle_id,
        )
    ordered = _sort_localized_subtitles(next_subtitles)
    spoken_segments = reconcile_localized_spoken_segments(
        draft.localized_spoken_segments,
        ordered,
    )
    if patch.model_fields_set.issubset({"text", "tts_text"}):
        # A wording edit must not normalize timing/source membership elsewhere.
        # Spoken anchors can intentionally differ from display cue boundaries.
        previous = next(item for item in draft.localized_subtitles if item.subtitle_id == subtitle_id)
        current = next(item for item in ordered if item.subtitle_id == subtitle_id)
        spoken_changed = (previous.tts_text or previous.text) != (current.tts_text or current.text)
        reconciled = {item.segment_id: item for item in spoken_segments}
        spoken_segments = [
            segment.model_copy(update={"text": reconciled[segment.segment_id].text})
            if spoken_changed and segment.segment_id == current.spoken_segment_id
            and segment.segment_id in reconciled else segment
    ...
    invalidated = subtitle_tts_invalidation.invalidate_after_localized_subtitle_edit(
        draft,
        next_draft,
        subtitle_id,
    )
    return localization_tracks.invalidate_formal_quality_binding(invalidated)
...
def with_editorial_subtitle_collection(
    before: VideoLocalizationDraft,
    after: VideoLocalizationDraft,
    changed_ids: set[str],
) -> VideoLocalizationDraft:
    """Reconcile changed subtitle bindings without rebuilding the whole draft.

    Temporarily empty spoken segments retain their original identity and
    paragraph metadata so undo can restore membership. The formal localization
    binding is invalidated; a later formal generation owns rebuilding it.
    """
    previous = {item.subtitle_id: item for item in before.localized_subtitles}
    current = {item.subtitle_id: item for item in after.localized_subtitles}
    changed_members = [
        item for item in [*before.localized_subtitles, *after.localized_subtitles]
        if item.subtitle_id in changed_ids
    ]
    segment_ids = {item.spoken_segment_id for item in changed_members if item.spoken_segment_id}
    cue_ids = {cue_id for item in changed_members for cue_id in _localized_subtitle_source_cue_ids(item)}
    scoped_segments = [item for item in after.localized_spoken_segments if item.segment_id in segment_ids]
    reconciled = {item.segment_id: item for item in reconcile_localized_spoken_segments(
        scoped_segments, [item for item in after.localized_subtitles if item.spoken_segment_id in segment_ids],
    )}
    structural_segments = {
        item.spoken_segment_id for item in changed_members
        if item.subtitle_id not in previous or item.subtitle_id not in current
        or any(getattr(previous[item.subtitle_id], field) != getattr(current[item.subtitle_id], field)
               for field in ("start_ms", "end_ms", "spoken_segment_id", "source_cue_ids", "source_word_ids"))
    }
    next_segments = [
        reconciled[item.segment_id]
        if item.segment_id in structural_segments and item.segment_id in reconciled
        else item.model_copy(update={"text": reconciled[item.segment_id].text})
        if item.segment_id in reconciled else item
        for item in after.localized_spoken_segments
    ]
    next_cues = []
    for cue in after.cues:
        if cue.cue_id not in cue_ids:
            next_cues.append(cue)
            continue
        related = [item for item in after.localized_subtitles if cue.cue_id in _localized_subtitle_source_cue_ids(item)]
        update = {
        ...
        if not related or any(item.tts_text is not None for item in related):
            update["tts_recommended_text"] = "\n".join(
    ...
    result = after.model_copy(update={"localized_spoken_segments": next_segments, "cues": next_cues})
    for subtitle_id in changed_ids.intersection(previous):
        result = subtitle_tts_invalidation.invalidate_after_localized_subtitle_edit(before, result, subtitle_id)
    return localization_tracks.invalidate_formal_quality_binding(result)


def reconcile_localized_spoken_segments(
    segments: list[VideoLocalizationSpokenSegment],
    subtitles: list[VideoLocalizationSubtitleCue],
) -> list[VideoLocalizationSpokenSegment]:
    """Keep the spoken track derived from its current display-subtitle members.

    Display subtitles own membership once that track exists.  Text edits,
    timing edits, splits, and deletions therefore update the same segment
    fields atomically; a segment with no remaining members is removed instead
    of surviving as stale planning input.
    """

    members_by_segment: dict[str, list[VideoLocalizationSubtitleCue]] = {}
    for subtitle in sorted(
        subtitles,
        key=lambda item: (item.start_ms, item.end_ms, item.subtitle_id),
    ):
        if subtitle.spoken_segment_id:
            members_by_segment.setdefault(
                subtitle.spoken_segment_id,
                [],
            ).append(subtitle)

    reconciled: list[VideoLocalizationSpokenSegment] = []
    for segment in segments:
        members = members_by_segment.get(segment.segment_id, [])
        if not members:
            continue
        reconciled.append(
            segment.model_copy(
...
def _assert_localized_subtitle_track_valid(
    ...
    *,
    ...
    for subtitle in ordered:
        if focus_subtitle_id and subtitle.subtitle_id != focus_subtitle_id:
            continue
