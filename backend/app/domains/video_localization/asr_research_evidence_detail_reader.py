"""Authoritative detail reader for managed research evidence."""
...
from app.domains.video_localization import (
    asr_research_evidence_operation_projection as projection,
...
def assemble_research_evidence_detail(
...
def _result_summary(
    ...
    *,
    ...
    step_result = summary["task_step_results"]["research"]
...
def _progress(status: str, steps) -> float:
    ...
    if any(
        step.status == "success"
        and step.step_id == "prepare_research_input"
        for step in steps
    ):
        return 0.55
    return 0.4
...
def _error_code(status: str, steps, attempts) -> str | None:
    ...
    return "VIDEO_LOCALIZATION_RESEARCH_FAILED"


def _error_message(status: str, error_code: str | None) -> str | None:
    if status != "failed":
        return None
    if error_code and "RESULT_UNKNOWN" in error_code:
        return "资料查询请求的结果暂时无法确认，系统没有自动重复请求。"
    return "资料查询未完成，请检查搜索与模型配置后重试。"


__all__ = ["assemble_research_evidence_detail"]
