...
AUTOSAVE_PATH_NORMALIZATION_MARKER = ".project-paths-v1-normalized"
...
def write_project_snapshot(
    ...
    *,
    ...
    root = media_assets.ensure_project_video_localization_dir(
        project.project_id
    )
    directories = ensure_project_layout(project.project_id)
    saved_at = str(getattr(project, "updated_at", "") or now_iso())
    ...
    manifest = {
        ...
        "repository_revision": int(
            getattr(project, "_repository_revision", 0)
        ),
        "saved_at": saved_at,
        "storage": {
            "root": str(root),
    ...
    if create_autosave:
        _write_json(
            _autosave_path(directories["autosave"]),
            portable_manifest,
        )
        _prune_autosaves(
            directories["autosave"],
            keep=AUTOSAVE_KEEP_LIMIT,
        )
    return manifest
...
def has_project_snapshot(project_id: str) -> bool:
    """Check recovery evidence without parsing, creating, or migrating storage."""
    ...
    try:
        if (root / "project.json").is_file():
            return True
        autosave_dir = root / "autosave"
        return autosave_dir.is_dir() and any(
            autosave_dir.glob("*-project.json")
        )
    except OSError:
        return False
...
def read_project_package(root: Path) -> dict[str, Any] | None:
    ...
    try:
        ...
        if not isinstance(draft_payload, dict):
            return None
        external_paths = _external_paths(draft_payload, root)
    ...
    return {
        ...
        "saved_at": _saved_at(payload, manifest_path),
    }


def _saved_at(payload: dict[str, Any], manifest_path: Path) -> str:
    ...
    if isinstance(value, str):
        try:
            ...
            pass
    try:
        return datetime.fromtimestamp(manifest_path.stat().st_mtime).isoformat(timespec="microseconds")
    except OSError:
        return now_iso()
...
def ensure_project_layout(project_id: str) -> dict[str, Path]:
    root = media_assets.ensure_project_video_localization_dir(project_id)
    directories = {
        "source": root / "source",
        "audio": root / "audio",
        "stems": root / "stems",
        ...
        "visual_evidence": root / "visual-evidence",
...
def _write_json(path: Path, payload: dict[str, Any]) -> None:
    ...
    durable_files.atomic_write_bytes(path, content)
...
def _normalize_existing_autosaves(project_id: str, root: Path, autosave_dir: Path) -> None:
    marker = autosave_dir / AUTOSAVE_PATH_NORMALIZATION_MARKER
    if marker.exists():
        return
