...
from app.domains.video_localization import dub_subtitles, dubbing_production
...
# Localized cards are already capped at 28 visible units.  Keep one card as a
# single semantic line and let the player adapt visual wrapping to its actual
# viewport; inserting a backend hard break between Chinese characters can
# split complete words such as “着火” or “压根”.
LOCALIZED_SUBTITLE_MAX_LINE_UNITS = 28
...
def evaluate_quality_gate(draft: VideoLocalizationDraft) -> VideoLocalizationQualityGate:
    ...
    project_media = media_health.inspect_project_media(draft).health
    _check_asr_timing_quality(draft, blockers, warnings)
    _check_word_provenance(draft, blockers)
    _check_boundary_review_quality(draft, blockers, warnings)
    _check_asr_text_review_quality(draft, warnings)
    ...
    if project_media.source_video.status != "available":
        warnings.append(
            _issue(
                "SOURCE_MEDIA_MISSING"
                if project_media.source_video.status == "unconfigured"
    ...
    if (
        ...
        or project_media.stems_status != "complete"
    ):
    ...
    for cue in draft.cues:
        _check_cue_basics(
            ...
            check_dubbing_text=has_dubbing_work and not has_localized_track,
        )
...
def evaluate_localized_subtitle_track(
    subtitles: list[VideoLocalizationSubtitleCue],
) -> VideoLocalizationQualityGate:
    """Evaluate one candidate Chinese track without inspecting project state."""

    blockers: list[VideoLocalizationQualityIssue] = []
    warnings: list[VideoLocalizationQualityIssue] = []
    _check_localized_subtitles(subtitles, blockers, warnings)
    blockers = _finalize_issues(blockers)
    warnings = _finalize_issues(warnings)
    return VideoLocalizationQualityGate(
        status=(
...
def visible_subtitle_units(text: str) -> float:
    """Return the same display-width units used by formal track limits."""

    return _visible_units(text)
...
def _has_dubbing_work(draft: VideoLocalizationDraft) -> bool:
    if draft.reference_clips or draft.generated_candidates:
        return True
...
def dub_subtitle_export_blockers(
    ...
    *,
    ...
    _check_localized_subtitles(projected, blockers, warnings)
    _check_media_duration_bounds(
        draft.model_copy(update={"localized_subtitles": projected}),
        blockers,
        check_cues=False,
        check_localized=True,
    )
    return _finalize_issues(blockers)


def dubbing_production_export_blockers(
    draft: VideoLocalizationDraft,
    *,
    current_audio_sha256_by_clip_id: dict[str, str | None] | None = None,
) -> list[VideoLocalizationQualityIssue]:
    """Block reviewed-plan exports when the audible timeline is not deliverable.

    Projects created before the versioned production plan remain readable and
    exportable during migration.  Once a project enters planned production, it
    can never fall back to legacy export merely because a source edit
    invalidated its active plan.
    """

    return dubbing_production_timeline_blockers(
        draft,
        phase="delivery",
...
def dubbing_production_timeline_blockers(
    draft: VideoLocalizationDraft,
    *,
    phase: str = "timeline",
    current_audio_sha256_by_clip_id: dict[str, str | None] | None = None,
) -> list[VideoLocalizationQualityIssue]:
    """Return canonical plan/CQC blockers for one current timeline phase."""

    state = draft.dubbing_production
    if state.active_plan is None and not draft.localized_subtitles:
        return []
    if state.active_plan is None:
        return [
            _issue(
                "DUBBING_PLAN_REQUIRED",
                "原文或本土化内容已变化，请基于当前版本重新生成配音计划。",
                "blocker",
            )
        ]
    try:
        timeline_revision = (
            dubbing_production.dubbing_timeline_projection_revision(draft)
            if phase == "timeline"
            else dub_subtitles.freeze_workflow_input(draft, regeneration_mode="full").source_revision
        )
        payload = dubbing_production.build_current_timeline_audit_input(
            draft,
        ...
        report = dubbing_production.audit_timeline(payload)
    except (AppException, TypeError, ValueError) as exc:
        return [
            _issue(
                "DUBBING_PRODUCTION_AUDIT_INVALID",
                f"配音生产检查无法完成：{exc}",
                "blocker",
            )
        ]
    ...
    return _finalize_issues(
        [
            ...
            and finding.code in hard_delivery_codes
        ]
    )
...
def _check_localization_source_current(
    ...
    try:
        actual = localization_source.current_source_fingerprint(draft)
    except ValueError:
        actual = ""
    if actual != expected:
        blockers.append(
...
def _check_asr_text_review_quality(
    draft: VideoLocalizationDraft,
    warnings: list[VideoLocalizationQualityIssue],
) -> None:
    transcription = draft.transcription
    cycle = (
        transcription.transcript_quality_cycle
        if transcription
        and isinstance(transcription.transcript_quality_cycle, dict)
        else {}
    )
    step_results = cycle.get("task_step_results")
    if not isinstance(step_results, dict):
        return
    step = step_results.get("transcript_quality_gate")
    if not isinstance(step, dict) or step.get("status") != "warning":
        return
    summary = str(step.get("summary") or "").strip()
    warnings.append(
        _issue(
            "ASR_TEXT_REVIEW_RECOMMENDED",
            summary
            or "ASR 文字已采用当前最高概率结果继续处理，但仍有少量内容建议结合原音复听。",
            "warning",
        )
    )
...
def _check_asr_timing_quality(
    ...
    all_cues_manually_verified = bool(draft.cues) and all(
        cue_tools.timing_confirmation_is_current_for_draft(draft, cue) for cue in draft.cues
    )
    ...
    if transcription and transcription.raw_asr_incomplete_ranges:
        ...
        warnings.append(
            _issue(
                "ASR_INCOMPLETE_RANGES",
                (
                    f"有 {len(transcription.raw_asr_incomplete_ranges)} 段音频未生成源语言听写，"
                    f"合计约 {format_timeline_duration(missing_duration_ms)}；"
                    "这些范围不会生成字幕或配音。"
                ),
                "warning",
            )
    ...
    unverified_interpolated_cues = [
        ...
        and not cue_tools.timing_confirmation_is_current_for_draft(draft, cue)
    ]
    ...
    for cue in draft.cues:
        if "source_timing_zero_width" in cue.quality_flags:
            warnings.append(
                _issue(
                    "ASR_CUE_ZERO_WIDTH_ALIGNMENT",
                    "该字幕含保留的零宽词级锚点；其前后有效词已校核，但该词不表示可靠独立时长。",
                    "warning",
                    cue_id=cue.cue_id,
                )
            )
...
def current_transcription_alignment_audio_sha256(
    draft: VideoLocalizationDraft,
) -> str | None:
    """Return the current bytes fingerprint for the transcript alignment track."""

    transcription = draft.transcription
    if transcription is None or not transcription.alignment_source_track_id:
        return None
    return _current_track_fingerprint(
        draft,
        transcription.alignment_source_track_id,
        hash_cache={},
    )
...
def _current_track_audio_path(
    ...
    if source_track_id == "vocals":
        return resolution.paths.vocals
    elif source_track_id == "original":
        return resolution.paths.source_audio
    elif source_track_id == "background":
        return resolution.paths.background
    return None
...
def _cached_track_fingerprint(draft: VideoLocalizationDraft, source_track_id: str | None) -> str | None:
    ...
    if source_track_id == "original":
        selected = (
            ...
            .health.source_audio.selected_source
        )
        if selected == "source_media":
            return draft.source_media.audio_sha256
        if selected == "original_stem":
            return draft.stems.original_audio_sha256
    ...
    if avoided_pairs:
        ...
        warnings.append(
            _issue(
                ...
                f"最终字幕采用了 {len(avoided_pairs)} 处高置信度不建议切分的语义边界{examples}，建议复核断句",
                "warning",
            )
        )

    local_boundary_review_complete = (
        transcription.boundary_review_prompt_version
        == "boundary-review-local-v1"
        and transcription.boundary_review_status == "completed"
    )
    review_required_pairs = (
        []
        if local_boundary_review_complete
        else _review_required_selected_pairs(draft, selected_pairs)
    )
...
def _check_word_provenance(
    ...
    for cue in asr_cues:
        ...
        manual_text_correction = {
            "manual_text_edit",
            "protected_manual_edit",
        }.issubset(cue.quality_flags)
        if (
            not manual_text_correction
            and _normalize_provenance_text(cue.en_subtitle_text)
            != _normalize_provenance_text(referenced_text)
        ):
            text_mismatch_cues.append(cue.cue_id)

        if cue.start_ms is None or cue.end_ms is None:
            continue
        if cue_tools.timing_confirmation_is_current_for_draft(draft, cue):
            # Keep word IDs as source provenance while allowing an auditioned
            # human correction to supersede an imperfect aligner edge.
            continue
        subtitle_entries = transcription.subtitle_entry_by_word_id
        first_word = referenced_words[0]
        referenced_start_ms = subtitle_entries.get(
            first_word.word_id,
            first_word.start_ms,
        )
        ...
        blockers.append(
            _issue(
                ...
                _counted_cue_message(
                    "ASR 字幕出点超过媒体总时长 "
                    f"{format_timeline_duration(duration_ms, frame_rate=draft.source_media.frame_rate)}",
                    cue_ids,
                ),
                "blocker",
            )
...
def _check_localized_subtitles(
    ...
    for subtitle in subtitles:
        ...
        if (
            ...
            in subtitle.quality_flags
        ):
            warnings.append(
                _issue(
                    "LOCALIZED_SUBTITLE_STRONG_PAUSE_REVIEW_REQUIRED",
                    "这条本土化字幕跨过较长的人声停顿，建议人工听一下入点和出点",
                    "warning",
                    cue_id=subtitle.linked_cue_id,
                )
            )
...
def _check_cue_basics(
    ...
    *,
    ...
    if cue.review_status == "blocked":
        ...
        and "generated_by_asr" not in cue.quality_flags
    ):
