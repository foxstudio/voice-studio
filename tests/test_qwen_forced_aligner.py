...
def test_missing_checkpoint_stays_local_and_exposes_domestic_source(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("VOICE_STUDIO_MODELS_DIR", str(tmp_path))
...
def test_runtime_prefers_managed_engine_environment(tmp_path, monkeypatch):
    managed_python = (
        tmp_path
        / "data"
        / "engines"
    ...
    monkeypatch.delenv("VOICE_STUDIO_QWEN_ALIGN_PYTHON", raising=False)
    monkeypatch.setattr(
        qwen_forced_aligner.settings_store,
        "get",
    ...
    assert qwen_forced_aligner.runtime_python() == managed_python
