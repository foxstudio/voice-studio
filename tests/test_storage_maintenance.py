...
def test_background_maintenance_does_not_block_startup_and_runs_only_once(
    ...
    monkeypatch.setattr(storage_maintenance, "_background_thread", None)

    started = time.monotonic()
    assert storage_maintenance.start_background_maintenance() is True
    elapsed = time.monotonic() - started

    assert elapsed < 0.25
    assert entered.wait(timeout=2)
    assert storage_maintenance.background_maintenance_status()["state"] == "running"
    assert storage_maintenance.start_background_maintenance() is False

    release.set()
    assert storage_maintenance.wait_for_background_maintenance(timeout=2)
    status = storage_maintenance.background_maintenance_status()
    assert status["state"] == "completed"
    assert status["error_count"] == 0


def test_background_maintenance_reports_competing_cache_owner(
    tmp_path,
    monkeypatch,
):
    monkeypatch.setattr(
    ...
    def busy_lock(_path):
        raise storage_maintenance.LockUnavailableError("busy")
    ...
    assert storage_maintenance.wait_for_background_maintenance(timeout=2)
    status = storage_maintenance.background_maintenance_status()
    assert status["state"] == "skipped_active_instance"
    assert status["error_count"] == 0
