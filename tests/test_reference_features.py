...
def test_asr_selection_endpoint_exposes_effective_joint_provider(
    ...
    monkeypatch.setattr(vibevoice_model, "physical_memory_bytes", lambda: 128 * 1024**3)

    response = _client(tmp_path).get("/api/engines/asr-selection")

    assert response.status_code == 200
    payload = response.json()
    assert payload["engine_id"] == "vibevoice-asr-mlx-4bit"
    assert payload["diarization_engine_id"] == "vibevoice-asr-mlx-4bit"
    assert payload["policy"]["official_full_model_auto_selected"] is False


def test_vibevoice_install_endpoint_dispatches_managed_install(
    tmp_path: Path, monkeypatch
):
    monkeypatch.setattr(model_catalog, "automatic_download_allowed", lambda _engine_id: True)
    monkeypatch.setattr(
        vibevoice_model,
        "start_install",
        lambda engine_id: {
    ...
    response = _client(tmp_path).post(
        "/api/engines/installations/vibevoice-asr-4bit/install"
    )

    assert response.status_code == 202
    assert response.json()["engine_id"] == "vibevoice-asr-4bit"
...
def test_f5_run_isolated_uses_persistent_worker_by_default(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        engine_registry.engine_runner.settings_store,
        "get",
        lambda: AppSettings(device="cpu"),
    )
    ...
    assert Path(captured["python"]) == engine_virtualenv_python(root)
...
def test_qwen3_run_isolated_uses_external_persistent_worker_by_default(
    tmp_path: Path,
    monkeypatch,
):
    captured: dict = {}
    ...
    monkeypatch.setenv("VOICE_STUDIO_QWEN3_TTS_ROOT", str(root))
    monkeypatch.delenv("VOICE_STUDIO_QWEN3_TTS_PERSISTENT_WORKER", raising=False)
    monkeypatch.setattr(engine_registry.qwen3_tts_worker, "run", fake_run)

    result = engine_registry.run_isolated(
        "qwen3-tts-mlx-0.6b",
        {"text": "常驻加载测试", "output_path": "/tmp/qwen3.wav"},
        timeout=321,
    )

    assert result["output_path"] == "/tmp/qwen3.wav"
...
def test_uploaded_voice_waveform_uses_managed_cache_endpoint(tmp_path: Path):
    client = _client(tmp_path)
    ...
    first = client.get(
        f"/api/voices/files/{uploaded['file_id']}/waveform",
        params={"bins": 64},
    )
    second = client.get(
        f"/api/voices/files/{uploaded['file_id']}/waveform",
    ...
    assert first.json()["bins"] == 64
    assert len(first.json()["peaks"]) == 64
    assert first.json()["duration"] > 0


def test_missing_voice_file_waveform_returns_not_found(tmp_path: Path):
    client = _client(tmp_path)

    response = client.get(
        "/api/voices/files/missing/waveform",
        params={"bins": 64},
    )
...
def test_engine_registry_exposes_only_current_main_engines(tmp_path: Path, monkeypatch):
    ...
    assert "speaker_diarization" in by_id["vibevoice-asr-mlx-4bit"]["capabilities"]
