...
from app.services import audio_tools, batch_inference_runner, batch_queue, database, doubao_client, engine_manifests, engine_request_builder, inference_runner, qwen3_tts_paths, task_queue, voice_store
...
def test_qwen3_uses_engine_defaults_only_when_generation_parameters_are_omitted(tmp_path):
    defaults = qwen3_tts_paths.GENERATION_DEFAULTS

    single_omitted = task_queue._kwargs(
        GenerateRequest(text="Qwen3 single defaults", engine_id="qwen3-tts-mlx-0.6b"),
        str(tmp_path / "single-defaults.wav"),
    )
    batch_omitted = _batch_payload(
        BatchGenerateRequest(
            engine_id="qwen3-tts-mlx-0.6b",
            segments=[BatchSegmentInput(text="Qwen3 batch defaults")],
        ),
        tmp_path,
    )
    for payload in (single_omitted, batch_omitted):
        assert {key: payload[key] for key in defaults} == defaults
    ...
    single_explicit = task_queue._kwargs(
        GenerateRequest(
            text="Qwen3 single explicit boundaries",
            engine_id="qwen3-tts-mlx-0.6b",
            **explicit,
        ),
        str(tmp_path / "single-explicit.wav"),
    )
    batch_explicit = _batch_payload(
        BatchGenerateRequest(
            engine_id="qwen3-tts-mlx-0.6b",
            parameters=explicit,
            segments=[BatchSegmentInput(text="Qwen3 batch explicit boundaries")],
        ),
        tmp_path,
    )
    for payload in (single_explicit, batch_explicit):
        assert {key: payload[key] for key in explicit} == explicit
        *_, temperature, top_p, top_k, repetition_penalty, _max_tokens = (
            inference_runner._build_qwen3_tts_kwargs(**payload)
        )
        assert (temperature, top_p, top_k, repetition_penalty) == (
            explicit["temperature"],
            explicit["top_p"],
            explicit["top_k"],
            explicit["repetition_penalty"],
        )
...
def test_f5_fallback_subprocess_receives_the_resolved_device(tmp_path, monkeypatch):
    runtime_root = tmp_path / "F5-TTS"
    runtime_root.mkdir()
    monkeypatch.setenv("VOICE_STUDIO_F5_TTS_ROOT", str(runtime_root))
    captured: dict = {}

    def fake_run(command, _root, _environment):
        payload_path = Path(command[-1])
        captured.update(json.loads(payload_path.read_text(encoding="utf-8")))

    monkeypatch.setattr(inference_runner, "_run_external", fake_run)
    monkeypatch.setattr(inference_runner, "_external_python", lambda _root: sys.executable)
    monkeypatch.setattr(
        inference_runner,
        "_audio_meta",
        lambda _path, sample_rate: {"duration_ms": 100, "sample_rate": sample_rate},
    )

    inference_runner.run_f5_tts(
        output_path=str(tmp_path / "fallback.wav"),
        text="F5 fallback device",
...
def test_qwen3_speed_is_postprocessed_into_a_real_duration_change(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        inference_runner.qwen3_tts_paths,
        "model_dir",
        lambda kind, **_kwargs: model_dir,
    )
