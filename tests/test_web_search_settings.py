...
def test_general_web_search_extracts_direct_result_urls(monkeypatch):
    html = b"""
        <a class="result__a" href="//duckduckgo.com/l/?uddg=https%3A%2F%2Fseed.bytedance.com%2Fen%2Fseedance2_0&amp;rut=x">
          <strong>Seedance</strong> 2.0 - seed.bytedance.com
        </a>
        <td class="result-snippet">ByteDance AI video generation model</td>
    ...
    results = web_search.search_general_web("Seedance 2.0", limit=3)
...
def test_general_web_search_reports_access_challenge(monkeypatch):
    html = b"""
        <div class="anomaly-modal">
          <input name="image-check_example" type="checkbox">
        </div>
    """
    monkeypatch.setattr(
        web_search.urllib.request,
    ...
    with pytest.raises(AppException) as exc_info:
        web_search.search_general_web("Seedance 2.0", limit=3)

    assert exc_info.value.code == "WEB_SEARCH_CHALLENGE"
