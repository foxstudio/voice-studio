...
def _restore_database_path_after_test():
    """Prevent this module's temporary database from leaking to later tests."""
...
def test_cuda_is_a_valid_explicit_device_setting(tmp_path: Path):
    response = _client(tmp_path).patch("/api/settings", json={"device": "cuda"})
