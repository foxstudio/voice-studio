...
def test_omnivoice_local_snapshot_prefers_managed_model_directory(tmp_path, monkeypatch):
    ...
    assert inference_runner.omnivoice_local_snapshot(managed) == managed.resolve()
...
def test_single_runner_loads_omnivoice_from_local_snapshot(tmp_path, monkeypatch):
    ...
    inference_runner.run_omnivoice(
        text="测试",
...
def test_batch_runner_loads_omnivoice_from_local_snapshot(tmp_path, monkeypatch):
    ...
    results = batch_inference_runner.run_omnivoice(
        {
            "common": {"device": "cpu", "model_dir": str(snapshot)},
...
def test_batch_runner_keeps_stable_mps_loading_options(tmp_path, monkeypatch):
    import torch

    snapshot = _create_snapshot(tmp_path / "hub")
    generated = tmp_path / "generated.wav"
    generated.write_bytes(b"audio")
    calls = _install_fake_omnivoice(monkeypatch, generated)
    monkeypatch.setattr(
    ...
    results = batch_inference_runner.run_omnivoice(
        {
            "common": {"device": "mps", "model_dir": str(snapshot)},
            "segments": [{"segment_id": "segment-1", "text": "测试", "output_path": str(tmp_path / "batch.wav")}],
        }
    )
    ...
    assert calls == [
        (
            ...
            {"device_map": "mps", "attn_implementation": "eager", "dtype": torch.float32},
        )
    ]
    assert results[0]["status"] == "success"
