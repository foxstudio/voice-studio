...
def test_public_transcription_segments_keep_provider_speaker_evidence():
    segment = asr_service.normalize_segments(
        [
            {
                "start_ms": 0,
                "end_ms": 1000,
    ...
    assert segment.confidence == pytest.approx(0.92)
    assert segment.model_dump(exclude_none=True)["speaker_id"] == "S01"


def test_moss_is_exposed_by_the_public_asr_registry():
    assert "moss-transcribe-diarize-mlx" in asr_service.SUPPORTED_ENGINES
    provider = asr_service.PROVIDERS.get("moss-transcribe-diarize-mlx")
    assert isinstance(provider, moss_mlx.MossMlxProvider)
