...
def test_timing_quality_gate_does_not_invent_reading_speed_rules_for_imported_track():
    ...
    assert "LOCALIZED_SUBTITLE_DURATION_TOO_SHORT" not in _codes(gate.warnings)
    assert "LOCALIZED_SUBTITLE_CPS_HIGH" not in _codes(gate.warnings)
    assert "LOCALIZED_SUBTITLE_CPS_HARD_LIMIT" not in _codes(gate.warnings)
...
def test_timing_quality_gate_blocks_failed_alignment_before_cues_exist():
    ...
    assert "ASR_ALIGNMENT_FAILED" in _codes(gate.blockers)
    assert "ASR_TIMING_INTERPOLATED" in _codes(gate.warnings)
...
def test_timing_quality_gate_blocks_failed_alignment_but_only_warns_about_interpolation():
    draft = _draft(
...
def test_timing_quality_gate_warns_about_isolated_low_confidence_cue():
    ...
    issue = next(item for item in gate.warnings if item.code == "ASR_CUE_TIMING_LOW_CONFIDENCE")
...
def test_acoustic_subtitle_entry_is_used_for_word_coverage():
    cue = _cue("cue_onset", 140, 1000)
    cue["source_word_ids"] = ["word_01"]
    cue["en_subtitle_text"] = "Source"
    transcription = _transcription()
    transcription["subtitle_entry_by_word_id"] = {"word_01": 140}
...
def test_timing_quality_gate_warns_about_semantically_indivisible_oversized_cue():
    ...
    issue = next(item for item in gate.warnings if item.code == "ASR_CUE_SEGMENTATION_LIMIT_EXCEEDED")
...
def test_timing_quality_gate_does_not_limit_localized_subtitle_duration_or_cps():
    draft = _draft(
    ...
    }.isdisjoint(_codes(gate.warnings))
    assert not gate.blockers


def test_timing_quality_gate_does_not_warn_for_duration_or_cps_targets():
    draft = _draft(
...
def test_quality_gate_blocks_selected_boundary_rejected_by_semantic_review():
    ...
    assert "ASR_SELECTED_BOUNDARY_REVIEW_AVOIDED" in _codes(gate.warnings)
...
def test_completed_local_boundary_review_does_not_require_legacy_semantic_records():
    transcription = _transcription()
    transcription.update(
        {
            "boundary_review_status": "completed",
            "boundary_review_prompt_version": "boundary-review-local-v1",
            "boundary_reviews": [],
            "words": [
                {**transcription["words"][0], "word_id": "word_01", "text": "The", "start_ms": 0, "end_ms": 500},
                {**transcription["words"][0], "word_id": "word_02", "text": "skill", "start_ms": 500, "end_ms": 1000},
    ...
    assert "ASR_SELECTED_BOUNDARY_UNREVIEWED" not in _codes(gate.warnings)
...
def test_quality_gate_allows_explicit_manual_source_text_correction():
    transcription = _transcription()
    transcription["words"] = [
        {
            **transcription["words"][0],
    ...
    assert "ASR_CUE_TEXT_WORD_MISMATCH" not in _codes(gate.blockers)
...
def test_generated_asr_risk_uses_specific_issue_without_generic_review_duplicate():
    cue = _cue("cue_0001", 0, 1000)
    cue["review_status"] = "needs_review"
    cue["quality_flags"] = [
        "generated_by_asr",
    ...
    gate = evaluate_quality_gate(
        _draft(cues=[cue], localized_subtitles=[], transcription=None)
    )

    assert "ASR_CUE_SEGMENTATION_LIMIT_EXCEEDED" in _codes(gate.warnings)
    assert "CUE_NEEDS_REVIEW" not in _codes(gate.warnings)


def test_quality_gate_keeps_transcript_text_review_advisory_visible():
    transcription = _transcription()
    transcription["transcript_quality_cycle"] = {
        "task_step_results": {
            "transcript_quality_gate": {
                "status": "warning",
                "summary": "已自动继续校时，建议复听 2 处。",
                "metrics": [
                    {"label": "建议复听", "value": "2"},
                    {"label": "阻断项", "value": "0"},
                ],
    ...
    issue = next(
        ...
        if item.code == "ASR_TEXT_REVIEW_RECOMMENDED"
    )
    assert "2 处" in issue.message
