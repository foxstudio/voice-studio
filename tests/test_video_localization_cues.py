...
def test_normalized_flags_remove_stale_speaker_review_when_analysis_is_not_required():
    ...
    normalized = cues.with_normalized_quality_flags(draft)
    normalized = cues.without_stale_generated_asr_review_status(normalized)

    assert normalized.cues[0].review_status == "ready"
    assert "needs_speaker_assignment" not in normalized.cues[0].quality_flags


def test_normalized_flags_keep_speaker_review_for_unassigned_multi_voice_cue():
    clusters = [
        VideoLocalizationSpeakerCluster(
    ...
    assert "needs_speaker_assignment" in normalized.cues[0].quality_flags
...
def test_ready_generated_asr_cue_remains_replaceable_on_rerun():
    draft = VideoLocalizationDraft(
        cues=[
            {
                "cue_id": "cue_0001",
                "start_ms": 0,
                "end_ms": 1000,
                "en_subtitle_text": "Clean generated subtitle",
                "review_status": "ready",
                "quality_flags": ["generated_by_asr", "engine:qwen3-asr-mlx"],
            }
        ]
    ...
    assert cues.is_replaceable_asr_candidate(draft.cues[0])


def test_old_blanket_asr_review_status_is_migrated_but_real_risks_remain():
    draft = VideoLocalizationDraft(
        cues=[
            {
                "cue_id": "cue_0001",
                "review_status": "needs_review",
                "quality_flags": ["generated_by_asr", "timing:high"],
            },
            {
                "cue_id": "cue_0002",
                "review_status": "needs_review",
    ...
    normalized = cues.without_stale_generated_asr_review_status(draft)

    assert [cue.review_status for cue in normalized.cues] == [
        "ready",
        "needs_review",
        "needs_review",
    ]
