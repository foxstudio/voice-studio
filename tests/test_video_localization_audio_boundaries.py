...
def test_audio_boundary_analysis_reports_every_adjacent_word_gap(monkeypatch):
    sample_rate = 1000
    ...
    features, metadata = audio_boundaries.analyze_word_boundaries(
        "source.wav",
        [_word("word_000001", 100, 400), _word("word_000002", 430, 700)],
    )

    assert len(features) == 1
    assert features[0].gap_ms == 30
    assert features[0].analysis_version == "energy-pause-v2"
    assert metadata["boundary_count"] == 1
    assert metadata["threshold_count"] == 3


def test_audio_boundary_analysis_does_not_fail_on_consecutive_point_anchors(
    monkeypatch,
):
    sample_rate = 1000
    audio = np.zeros(1000, dtype=np.float32)
    ...
    features, metadata = audio_boundaries.analyze_word_boundaries(
        "source.wav",
        [
            _word("word_000001", 100, 300),
            _word("word_000002", 300, 300),
            _word("word_000003", 300, 300),
            _word("word_000004", 300, 500),
        ],
    )

    assert metadata["status"] == "completed"
    assert metadata["boundary_count"] == 3
    assert len(features) == 3


def test_candidate_audio_uses_consensus_across_energy_thresholds(monkeypatch):
    sample_rate = 1000
    ...
    evidence = audio_boundaries.analyze_dubbing_candidate_audio("candidate.wav")

    internal = [gap for gap in evidence.gap_evidence if gap.kind == "internal"]
    assert internal
    assert internal[0].duration_ms >= 30
    assert any("consensus" in evidence_id for evidence_id in internal[0].evidence_ids)
