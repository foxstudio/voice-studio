...
def test_task_page_applies_voice_filter_without_search_text(isolated_db):
    now = datetime.now(timezone.utc)
    _save_task(1, status=TaskStatus.success, created_at=now)
    _save_task(2, status=TaskStatus.success, created_at=now)

    response = TestClient(app).get(
        "/api/tasks/page",
        params={"voice_ids": "voice-b"},
    )

    assert response.status_code == 200
    assert response.json()["total"] == 1
    assert response.json()["items"][0]["task_id"] == "task-02"
...
def test_download_sequences_are_computed_in_one_ranked_query(
    isolated_db,
    monkeypatch,
):
    ...
    targets = [
        task_queue.get_task("task-03"),
        task_queue.get_task("task-17"),
        task_queue.get_task("task-29"),
    ]
    assert all(targets)
    ...
    monkeypatch.setattr(db, "conn", traced_conn)

    sequences = task_queue.task_download_sequences(
        [task for task in targets if task is not None]
    )
