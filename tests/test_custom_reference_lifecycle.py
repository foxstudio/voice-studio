...
def test_ttl_cleanup_scans_each_reference_table_once(
    isolated_store,
    monkeypatch,
):
    now = datetime(2026, 7, 13, 12, 0, 0)
    old = (now - timedelta(hours=2)).isoformat(timespec="seconds")
    first = _custom_file("ttl-first", created_at=old)
    second = _custom_file("ttl-second", created_at=old)
    _task("ttl-second-protector", second.path)
    original_list_all = db.list_all
    calls: dict[str, int] = {}

    def tracked_list_all(table, *args, **kwargs):
        calls[table] = calls.get(table, 0) + 1
        return original_list_all(table, *args, **kwargs)

    monkeypatch.setattr(db, "list_all", tracked_list_all)

    deleted = custom_reference_store.cleanup_orphaned_uploads(
        ttl_seconds=3600,
        now=now,
    )

    assert deleted == [first.file_id]
