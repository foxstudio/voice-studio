...
def test_evaluation_exposes_timeline_integrity_failures_without_reading_speed_limits():
    ...
    assert report["predicted"]["invalid_duration_count"] == 0
    assert "under_800ms_count" not in report["predicted"]
    assert "over_12cps_count" not in report["predicted"]
