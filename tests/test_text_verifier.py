...
def test_verifier_warns_when_expected_text_is_covered_but_extra_speech_is_present():
    report = text_verifier.verify_transcript(
        expected_text="所以我一次性要了几种不同的世界。",
        transcript_text="It has to，所以我一次性要了几种不同的世界。",
    )

    assert report.coverage == 1.0
    assert report.similarity < 0.9
    assert report.status == "warning"
    assert "额外内容" in report.warnings[0]
