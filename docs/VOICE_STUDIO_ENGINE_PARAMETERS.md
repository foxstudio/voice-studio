...
- 声音设计：只在统一模型目录的 `qwen3-tts-mlx-0.6b/Qwen3-TTS-12Hz-0.6B-VoiceDesign-8bit`（或兼容的旧版运行时 `models/`）存在时展示；只填 `voice_design_prompt`，不混用 `speaker_id` 或参考音色，避免 VoiceDesign、CustomVoice、Base 三条路线互相覆盖。
