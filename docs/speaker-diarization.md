...
1. The same versioned audio artifact fans out to two independent branches:
   primary ASR and MOSS anonymous speaker diarization.
2. MOSS produces anonymous speaker ranges (`S01`, `S02`, ...), then CAM++
   compares representative clean ranges and conservatively merges likely false
   splits.
3. The two branches join only after their audio hash and source track match.
   Speaker ranges are mapped to ASR segments by time overlap, then the transcript
   coverage gate runs.
4. Text review and forced alignment continue from the joined transcript.
5. Speaker ranges are mapped to forced-aligned words by time overlap.
6. Subtitle segmentation may not cross a confirmed speaker-cluster boundary.
7. Anonymous acoustic clusters are bound to stable project speakers
   (`speaker_01`, ...).
8. Localization context and web research may propose identity candidates,
   topics, and background evidence, but may not silently turn a candidate into
   a confirmed identity.

Development mode can run raw ASR and speaker diarization as separate operations.
Each writes a versioned snapshot outside the formal product data directory and
must not update formal subtitles, transcription state, or project speakers.
Formal runs never read those development snapshots.

Speaker count is automatic by default. The current MOSS MLX CLI does not support
an exact, minimum, or maximum speaker-count constraint. Optional
`min_speakers`/`max_speakers` values are quality guidance only: an out-of-range
result is marked for review, but acoustic clusters are never force-merged merely
...
上述是当前默认位置。修改 Voice Studio 的数据根目录或模型根目录后，MOSS 和
CAM++ 会跟随新目录；也可用 `VOICE_STUDIO_MOSS_RUNTIME_ROOT`、
`VOICE_STUDIO_MOSS_MODEL_DIR`、`VOICE_STUDIO_CAMPPLUS_ROOT` 和
`VOICE_STUDIO_CAMPPLUS_MODEL` 显式复用已有外部安装。
