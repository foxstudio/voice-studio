...
| 推荐模型目录 | `~/VoiceStudio/models/qwen3-tts-mlx-0.6b/` |
| 采样率 | 24000 Hz |
...
只有统一模型根下存在 `qwen3-tts-mlx-0.6b/Qwen3-TTS-12Hz-0.6B-VoiceDesign-8bit` 时，Voice Studio 才展示 `voice_design_prompt` 和“Qwen3 声音设计”预设。旧版运行时目录中的 `models/Qwen3-TTS-12Hz-0.6B-VoiceDesign-8bit` 仍可兼容读取。填写 `voice_design_prompt` 时，Voice Studio 使用 VoiceDesign 模型，把这段描述作为官方 `instruct` 参数；它会接管预置音色和风格指令。
