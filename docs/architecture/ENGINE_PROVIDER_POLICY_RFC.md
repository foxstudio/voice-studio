...
- `indextts-v2`、`omnivoice` 和 `confucius4-mlx-int8` 默认复用同引擎常驻进程，
  `VOICE_STUDIO_LOCAL_PERSISTENT_WORKER=0` 可回退到原来的单次隔离子进程。模型缓存键必须包含
  引擎、模型路径和设备，停止引擎或应用退出时必须终止对应 Worker。
  默认空闲 900 秒后自动回收，`VOICE_STUDIO_LOCAL_WORKER_IDLE_SECONDS`
  可调整时间，设为 `0` 表示只在停止引擎或应用退出时回收。
- persistent worker 的 stdout 响应由后台读取线程写入队列，不对子进程管道调用
  `select`；这是 POSIX 和 Windows 共用的超时、取消与异常退出语义。
