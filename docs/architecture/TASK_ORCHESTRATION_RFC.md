...
云端/MiMo 任务在进入 Provider 提交窗口前必须先持久化“结果可能不确定”的状态。
Provider 支持幂等提交或结果回查时，还必须持久化并复用其真实接受的
`provider_request_id` / idempotency key。本地内容 fingerprint 只能识别输入，
不能替代 Provider 幂等键，也不能作为自动重放依据。
...
1. 重启恢复：
   - queued 可恢复为 queued
   - 本地 running/postprocessing/retrying 可恢复为 queued 且写恢复注记
   - 云端 running/postprocessing/retrying 或 uncertain 不自动恢复，转入失败和人工确认
...
- 当前代码中云端计费幂等与恢复回查能力未完全齐备，需在 Phase 2 落地：
  - 单句豆包已持久化并复用 request ID；MiMo 和批量逐 segment 仍需 Provider
    可接受的 request ID / idempotency key；
  - 批量队列已在 Provider 边界前持久化 batch-level uncertain，重启不自动重放，
    且确认重试只提交非成功 segment；仍需逐 segment attempt 与人工裁决记录；
  - 重启重放决策的判定 API；
