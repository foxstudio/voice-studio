<script lang="ts">
	...
	import LocalizationAiSettings from './components/LocalizationAiSettings.svelte';
	...
	import AsrSelectionSettings from './components/AsrSelectionSettings.svelte';
	...
	const searchItems: { section: SectionId; target: string; title: string; description: string; keywords: string; content?: string }[] = [
		...
		{ section: 'ai', target: 'ai-title', title: '语言模型、本土化策略与联网检索', description: '管理模型连接、视频本土化分工和按需背景检索', keywords: 'AI 助手 语言模型 LLM 本土化 自动 质量 成本 输出格式 搜索 Tavily Wikipedia SearXNG', content: '配置名称 接口协议 Base URL API Key 模型 ID 启用此服务 默认服务 全文理解 中文创作 质检 时间映射 按需联网检索' },
		...
		{ section: 'common', target: 'device', title: '计算设备', description: '自动、NVIDIA CUDA、Apple MPS 或 CPU', keywords: '性能 芯片 gpu cpu cuda mps' },
		...
		{ section: 'common', target: 'default-asr-engine', title: '默认语音识别', description: '自动选择或固定 ASR 引擎', keywords: 'asr vibevoice qwen 说话人 转录 默认 模型' },
		{ section: 'advanced', target: 'stem-overlap', title: '分离精细度', description: '提高人声和背景声分离精细度', keywords: 'BS RoFormer 人声 背景声 overlap 重叠' },
		{ section: 'advanced', target: 'stem-chunk-duration', title: '长视频分段', description: '控制长视频分离时的内存占用', keywords: 'BS RoFormer 长视频 分段 内存 chunk' },
		{ section: 'advanced', target: 'cache-dir', title: '缓存目录', description: 'ASR、波形、对齐和视频预览缓存位置', keywords: 'cache asr waveform align preview path 路径 视频预览' },
		{ section: 'advanced', target: 'preview-cache-mode', title: '视频预览缓存', description: '时间线鼠标预览的画质、密度和容量上限', keywords: '视频 预览 缓存 帧 画质 容量 空间 timeline scrub' },
	...
	let availableDevices = $state<string[]>(['cpu']);
	...
	async function load() {
		...
		if (healthResult.status === 'fulfilled') {
			availableDevices = healthResult.value.platform_capabilities.framework_devices?.torch
				?? healthResult.value.platform_capabilities.available_devices;
		}
		loading = false;
	}
			...
			<div class="settings-toolbar">
				<div class="search-shell" class:focused={searchFocused}>
					...
					{#if searchFocused && normalizedQuery}
						<div class="search-results menu-scroll-region" id="settings-search-results" role="listbox">
			...
			<div class="settings-content">
				{#if activeSection === 'common'}
					<section class="content-section" aria-labelledby="common-title" oninput={markDirty} onchange={markDirty}>
						...
						<div class="setting-list common-settings">
							...
							<AsrSelectionSettings {settings} onChange={markDirty} />
							...
							<SettingsRow icon={Cpu} title="计算设备" description="支持设备选择的本地引擎会使用此设置；不可用设备不会静默降级" controlId="device">
								<select id="device" aria-label="计算设备" bind:value={settings.device}><option value="auto">自动选择（推荐）</option><option value="cuda" disabled={!availableDevices.includes('cuda')}>NVIDIA CUDA{availableDevices.includes('cuda') ? '' : ' · 当前不可用'}</option><option value="mps" disabled={!availableDevices.includes('mps')}>Apple 芯片 MPS{availableDevices.includes('mps') ? '' : ' · 当前不可用'}</option><option value="cpu">CPU</option></select>
							</SettingsRow>
				...
				{:else if activeSection === 'ai'}
					<section class="content-section" aria-labelledby="ai-title">
						<div class="section-heading"><div><h2 id="ai-title">语言模型与业务策略</h2><span>先管理可用连接，再为视频本土化选择自动分工或阶段专用模型。</span></div></div>
						<LlmSettings />
						<LocalizationAiSettings {settings} onChange={markDirty} />
						<SearchSettings />
					</section>
				...
				{:else if activeSection === 'advanced'}
					<section class="content-section" aria-labelledby="advanced-title" oninput={markDirty} onchange={markDirty}>
						...
						<div class="setting-list path-list">
							...
							<SettingsRow icon={MonitorCog} title="模型目录" description={settings.model_dir_source === 'environment' ? '由 VOICE_STUDIO_MODELS_DIR 统一管理；请修改启动变量' : '本地模型权重的统一保存位置；支持 ~ 和环境变量'} controlId="model-dir">
								{#if settings.model_dir_source === 'environment'}
									<input id="model-dir" value={settings.model_dir_effective} readonly />
								{:else}
									<input id="model-dir" bind:value={settings.model_dir} />
								{/if}
							</SettingsRow>
							<SettingsRow icon={SlidersHorizontal} title="分离精细度" description="BS-RoFormer 推理片段的重叠次数；8 是已试听方案，越高越慢" controlId="stem-overlap">
								<div class="number-with-unit"><input id="stem-overlap" type="number" min="2" max="50" step="1" bind:value={settings.stem_separation_overlap} /><span>次</span></div>
							</SettingsRow>
							<SettingsRow icon={Gauge} title="长视频分段" description="每段保留上下文后独立处理；300 秒兼顾内存与连续性" controlId="stem-chunk-duration">
								<div class="number-with-unit"><input id="stem-chunk-duration" type="number" min="60" max="1800" step="30" bind:value={settings.stem_separation_chunk_duration_seconds} /><span>秒</span></div>
							</SettingsRow>
							<SettingsRow icon={HardDrive} title="缓存目录" description="ASR、波形、对齐和视频时间线预览缓存" controlId="cache-dir"><input id="cache-dir" bind:value={settings.cache_dir} /></SettingsRow>
							<SettingsRow icon={Gauge} title="视频预览缓存" description="自动模式会根据视频长度和磁盘余量调整帧密度" controlId="preview-cache-mode">
								<select id="preview-cache-mode" bind:value={settings.video_preview_cache_mode}>
									<option value="auto">自动平衡</option>
									<option value="compact">节省空间</option>
									<option value="quality">高质量</option>
								</select>
							</SettingsRow>
							<SettingsRow icon={HardDrive} title="预览缓存容量" description="达到上限后优先回收最久未使用的项目缓存" controlId="preview-cache-limit">
								<div class="number-with-unit"><input id="preview-cache-limit" type="number" min="0.5" max="64" step="0.5" bind:value={settings.video_preview_cache_max_gb} /><span>GB</span></div>
							</SettingsRow>
...
<style>
	...
	.search-results { position: absolute; z-index: 20; top: calc(100% + 8px); left: 0; right: 0; max-height: min(360px, calc(100vh - 140px)); overflow-x: hidden; overflow-y: auto; overscroll-behavior: contain; border: 1px solid #2a3441; border-radius: 11px; background: rgba(17, 22, 30, .98); box-shadow: 0 18px 48px rgba(0, 0, 0, .42); }
	...
	.number-with-unit input { width: 112px; }
	.number-with-unit span { color: #8793a1; font-size: 11px; }
