...
describe('reference audio trim boundaries', () => {
	...
	it('invalidates a restored managed source even when no browser File exists', () => {
		const restored = referenceAudioDraftFromLegacyState(legacyState({
			customVoicePreviewUrl: '/api/voices/files/clip-1/audio',
			customVoiceSourceAudioPath: 'voices/files/source-1.wav',
			customVoiceReferenceAudioPath: 'voices/files/clip-1.wav'
		}));
		restored.source.previewUrl = '/api/voices/files/source-1/audio';

		const patch = legacyCustomVoicePatchFromDraft(withReferenceAudioTrim(restored, 4_000, 9_000));

		expect(patch.customVoiceSourceFileId).toBe('source-1');
		expect(patch.customVoiceSourceAudioPath).toBe('voices/files/source-1.wav');
		expect(patch.customVoicePreviewUrl).toBe('/api/voices/files/source-1/audio');
		expect(patch.customVoiceFileId).toBe('');
		expect(patch.customVoiceReferenceAudioPath).toBe('');
		expect(patch.customVoiceTranscript).toBe('');
		expect(patch.customVoiceTranscriptionId).toBe('');
		expect(patch.customVoiceConfirmed).toBe(false);
	});
});
...
describe('reference editor draft reentry', () => {
	it('retains unapplied selection status and source metadata through the store adapter', () => {
		const original = referenceAudioDraftFromLegacyState(legacyState());
		const edited = withReferenceAudioTrim(original, 3000, 7000);
		const returned = referenceAudioDraftFromLegacyState(legacyCustomVoicePatchFromDraft(edited));
		expect(returned.selectionDirty).toBe(true);
		expect(returned.trim).toEqual({ startMs: 3000, endMs: 7000 });
		expect(returned.source.durationMs).toBe(30000);
		expect(returned.source.fileId).toBe('source-1');
		expect(returned.transcript.text).toBe('');
		expect(returned.confirmed).toBe(false);
	});
});
