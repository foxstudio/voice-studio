<script lang="ts">
	...
	import { TooltipSession, computeTooltipPosition, type TooltipPlacement } from '$lib/components/shared/hover-tooltip';
	...
	const hoverSession = new TooltipSession<HTMLElement>({
		show: () => {
			open = true;
			void positionPanel();
		},
	...
	function enterTrigger() {
		if (rootEl) hoverSession.enterTarget(rootEl);
	}

	function leaveTrigger() {
		if (rootEl) hoverSession.leaveTarget(rootEl);
	}
	...
	async function positionPanel() {
		...
		panelStyle = `left:${position.left}px; top:${position.top}px;`;
	}

	onMount(() => {
		const closeOutside = (event: PointerEvent) => {
			const target = event.target as Node | null;
			if (target && (rootEl?.contains(target) || panelEl?.contains(target))) return;
			hoverSession.close();
		};
		const closeOnEscape = (event: KeyboardEvent) => {
			if (event.key === 'Escape') hoverSession.close();
		};
...
<span
	...
	onfocusin={enterTrigger}
	onfocusout={leaveTrigger}
>
	...
	<span
		...
		style={panelStyle}
		role="tooltip"
		onmouseenter={() => hoverSession.enterLayer()}
		onmouseleave={() => hoverSession.leaveLayer()}
		onfocusin={() => hoverSession.enterLayer()}
		onfocusout={() => hoverSession.leaveLayer()}
	>
