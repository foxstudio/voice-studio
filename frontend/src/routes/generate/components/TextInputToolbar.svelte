<script lang="ts">
	import { ArrowLeft, Hash, Link2, Scissors, Send, Wand2 } from 'lucide-svelte';
	...
	type Props = {
		...
		associationChecked?: boolean;
		associationDetail?: string;
		associationReturnHref?: string;
...
<div class="tool-row">
	...
	<div class="submit-actions">
		{#if associationDetail}
			<label class="association-toggle" data-tooltip={associationDetail}>
				<input
					type="checkbox"
					checked={associationChecked}
					onchange={(event) => onAssociationChange((event.currentTarget as HTMLInputElement).checked)}
				/>
				<Link2 size={13} />
				<span>关联</span>
			</label>
			{#if associationReturnHref}
				<a class="btn tool-btn return-project-btn" href={associationReturnHref} aria-label="返回视频本土化项目" data-tooltip="返回视频本土化项目">
					<ArrowLeft size={14} />
				</a>
			{/if}
		{/if}
...
<style>
	...
	.submit-actions { display: flex; align-items: center; justify-content: flex-end; gap: 6px; margin-left: auto; flex: 0 0 auto; }
	.association-toggle { display: inline-flex; align-items: center; gap: 5px; height: 28px; padding: 0 8px; box-sizing: border-box; border: 1px solid rgba(87, 208, 200, .26); border-radius: 6px; background: rgba(87, 208, 200, .06); color: var(--muted); font-size: 11px; line-height: 1; white-space: nowrap; cursor: pointer; transition: border-color 120ms ease, background 120ms ease, color 120ms ease; }
	.association-toggle:hover { border-color: rgba(87, 208, 200, .46); background: rgba(87, 208, 200, .1); color: var(--text); }
	.association-toggle:focus-within { outline: 2px solid rgba(87, 208, 200, .24); outline-offset: 1px; }
	.association-toggle input { width: 13px; height: 13px; margin: 0; accent-color: var(--accent-2); }
	.association-toggle:has(input:checked) { border-color: rgba(87, 208, 200, .48); background: rgba(87, 208, 200, .12); color: var(--text); }
	.return-project-btn { width: 28px; padding: 0; }
	.generate-inline-btn { flex: 0 0 auto; }
	@media (max-width: 640px) {
		.tool-row { align-items: stretch; flex-direction: column; }
		.submit-actions { width: 100%; margin-left: 0; }
		.generate-inline-btn { flex: 1 1 auto; }
	}
</style>
