...
import {
	...
	taskIsFailed,
	taskCanDelete,
	taskUsesVideoLocalizationReview,
	cloudReplayConfirmationMessage,
	longformCloudReplayConfirmationMessage,
...
import type { EngineDetail, GenerationTask, LongformTask, ParameterSchema, VoiceAsset } from '$lib/api/types';
...
describe('queue presentation does not invent dispatch order', () => {
	it('shows waiting state without a position inferred from display order', () => {
		const waiting = makeTask({ task_id: 'waiting', status: 'queued' });
		const another = makeTask({ task_id: 'another', status: 'queued' });
		for (const visibleOrder of [[waiting, another], [another, waiting], []]) {
			expect(progressLabel(waiting, visibleOrder)).toBe('等待');
			expect(taskStatusPillLabel(waiting, { processing: 1, waiting: 2 }, visibleOrder)).toBe('排队中');
			expect(taskStageLabel(waiting, visibleOrder)).toBe('等待后台安排');
			expect(taskRuntimeHint(waiting, { processing: 1, waiting: 2 }, visibleOrder)).toBe('按项目与优先级调度，当前任务结束后安排。');
		}
	});

	it('keeps actual running engine and waiting count without naming a next engine', () => {
		const running = makeTask({ status: 'running', engine_id: 'omnivoice' });
		const waiting = makeTask({ status: 'queued', engine_id: 'indextts-v2' });
		const engines = engineMap([engineDetail('omnivoice', []), engineDetail('indextts-v2', [])]);
		expect(queueSummaryText({ processing: 1, waiting: 3 }, [waiting, running], engines)).toBe('正在执行：omnivoice，后面等待 3 条。');
		expect(queueSummaryText({ processing: 0, waiting: 3 }, [waiting], engines)).toBe('等待 3 条任务，后台按项目与优先级调度。');
		expect(taskRuntimeHint(waiting, { processing: 0, waiting: 3 }, [waiting])).toBe('按项目与优先级调度，等待后台安排。');
		expect(queueSummaryText({ processing: 0, waiting: 0 }, [], engines)).toBe('');
	});

	it('preserves real running progress and terminal labels', () => {
		const running = makeTask({ status: 'running', progress: 0.4 });
		expect(progressLabel(running, [])).toBe('40%');
		expect(taskStatusPillLabel(running, { processing: 1, waiting: 0 }, [])).toBe('渲染中');
		expect(taskStageLabel(running, [])).toBe('声学推理');
		expect(taskStatusPillLabel(makeTask(), { processing: 0, waiting: 0 }, [])).toBe('成功');
	});
});
...
describe('task helpers', () => {
	it('requires an explicit warning before replaying an uncertain cloud result', () => {
		expect(
			cloudReplayConfirmationMessage(
				makeTask({ provider_state_uncertain: true })
			)
		).toContain('再次计费');
		expect(
			cloudReplayConfirmationMessage(
				makeTask({ provider_state_uncertain: false })
			)
		).toBe('');
	});

	it('requires an explicit warning before retrying cloud longform segments', () => {
		const task = {
			longform_task_id: 'longform-1',
			engine_id: 'mimo-v2.5-tts-preset'
		} as LongformTask;
		const cloudEngine = engineDetail(
		...
		expect(
			longformCloudReplayConfirmationMessage(
				task,
				engineMap([cloudEngine])
			)
		).toContain('再次计费');
		expect(
			longformCloudReplayConfirmationMessage(
				{ ...task, engine_id: 'indextts-v2' },
				engineMap([
					{
						...cloudEngine,
						manifest: {
							...cloudEngine.manifest,
							engine_id: 'indextts-v2',
							engine_type: 'local'
						},
						state: {
...
describe('requestFromTask', () => {
	...
	it('restores video localization association metadata from task fields', () => {
		const request = requestFromTask(makeTask({
			...
			parameters: { language: 'zh', output_format: 'wav' }
		}));
	...
	it('keeps bound video localization review out of the generic TTS verification flow', () => {
		expect(taskUsesVideoLocalizationReview(makeTask({
...
describe('resultDownloadName', () => {
	it('leaves the global sequence for the download endpoint', () => {
		const task = makeTask({
			input_text: '说每天雷打不动训练至少三四小时',
			parameters: { output_format: 'mp3' }
		});

		expect(resultDownloadName(task)).toBe('说每天雷打不动训练至少三四小时.mp3');
	});
	...
	it('sanitizes punctuation and limits long prompts', () => {
		...
		expect(resultDownloadName(task)).toBe('那-AI-应该怎么办-比如隐私-效率-成本-需要非常非常非常非常.wav');
	});

	it.each([
		['pcm', '原始-PCM.pcm'],
		['ogg_opus', 'OGG-Opus.ogg']
		...
		expect(resultDownloadName(task)).toBe(expectedName);
	});
