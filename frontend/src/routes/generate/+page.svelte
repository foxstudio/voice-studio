<script lang="ts">
	...
	async function restoreRequest(req: GenerateRequest) {
		...
		store.fromRequest(req, { preserveVideoLocalizationBinding: false });
		$store = { ...get(store) };
	...
	function videoLocalizationHandoffLabel(meta: VideoLocalizationHandoffMeta) {
		const action = meta.mode === 'tune_with_recipe' ? '带参数调试' : '仅带样音生成';
		return `${action} · ${meta.subtitle_id || meta.cue_id}`;
	}
	...
	function updateCustomVoiceTrimRange(startMs: number, endMs: number) {
		customVoiceTrimStart = Math.max(0, startMs / 1000);
		customVoiceTrimEnd = Math.max(customVoiceTrimStart + 0.1, endMs / 1000);
	...
	{#if videoLocalizationHandoff}
		<div class="handoff-banner">
			<div>
				...
				<span>{videoLocalizationHandoffLabel(videoLocalizationHandoff)}</span>
			</div>
			<Toggle
				compact
				checked={$store.requestBindToVideoLocalization}
				label="生成后关联回当前字幕"
				onChange={(checked) => ($store.requestBindToVideoLocalization = checked)}
			/>
