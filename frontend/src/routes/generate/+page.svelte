...
<main class="page generate-page">
	...
	<div class="workbench"><div class="panel stack compose-panel">
		...
		{#if !isSeedAudio && $store.showMoreParams && hasMoreParams}<div class="more-params-panel">
			{#if activeParamKeys.has('speaker_id')}<div class="field param-field" class:span-wide={hasSearchableSpeakerCatalog} class:field-muted={isQwen3TTS && !qwen3PresetRoute}><label class="param-label" for="spk">{isQwen3TTS ? '预置音色' : '音色'}</label><div class="param-control">{#if hasSearchableSpeakerCatalog}<div class="speaker-catalog-tools"><div class="gen-search-field speaker-search"><Search size={14} /><input bind:value={$store.speakerQuery} placeholder="搜索名称或 ID" />{#if $store.speakerQuery.trim()}<button class="gen-search-clear" type="button" onclick={() => ($store.speakerQuery = '')}><X size={13} /></button>{/if}</div><select class="speaker-gender" bind:value={$store.speakerGenderFilter}><option value="all">全部</option><option value="F">女声</option><option value="M">男声</option></select></div>{/if}{#if isDoubaoPreset}<input id="spk" list="doubao-speaker-options" bind:value={$store.speakerId} placeholder="选择或输入官方音色 ID" /><datalist id="doubao-speaker-options">{#each speakerChoices as o}<option value={o.value}>{o.label}</option>{/each}</datalist>{:else}<select id="spk" bind:value={$store.speakerId} disabled={isQwen3TTS && !qwen3PresetRoute}>{#each speakerChoices as o}<option value={o.value}>{o.label}</option>{/each}</select>{/if}{#if isEmotiVoice}<small>{$store.speakerCatalogLoading ? '读取中' : `结果 ${$store.speakerCatalog.length} 条`}</small>{:else if isDoubaoPreset}<small>{$store.speakerCatalogLoading ? '读取中' : `内置目录 ${$store.speakerCatalog.length} 个；也可直接输入账号已授权的官方音色 ID。`}</small>{:else if isQwen3TTS}<small>{qwen3PresetRoute ? '未选择本地/自定义音色时生效。填写声音描述后会改用 VoiceDesign。' : (qwen3VoiceDesignRoute ? '声音描述已接管，预置音色不参与本次生成。' : qwen3PresetDisabledText)}</small>{/if}</div></div>{/if}
