<script lang="ts">
	...
	import EmotionReferencePanel from './engine-ui/indextts-v2/EmotionReferencePanel.svelte';
	import {
		INDEX_TTS_ENGINE_ID,
		createDefaultIndexTtsEmotionState,
		type IndexTtsEmotionState
	} from './engine-ui/indextts-v2/state';
	import { validateIndexTtsEmotionState } from './engine-ui/indextts-v2/validation';
	import ReferenceAudioRangeEditor from './engine-ui/reference-audio/ReferenceAudioRangeEditor.svelte';
	...
	let submissionPreview: SubmissionPreview | null = $state(null);
	...
	let customVoiceFileInput = $state<HTMLInputElement>();
	...
	const customVoiceSourcePreviewUrl = $derived(seedAudioFileUrl($store.customVoiceSourceFileId));
	const customVoiceSourceDurationMs = $derived($store.customVoiceSourceDurationMs);
	const customVoiceTrimStart = $derived(($store.customVoiceTrimStartMs ?? 0) / 1000);
	const customVoiceTrimEnd = $derived(($store.customVoiceTrimEndMs ?? $store.customVoiceSourceDurationMs ?? 0) / 1000);
	const customVoiceSelectionDirty = $derived($store.customVoiceSelectionDirty);
	let videoLocalizationHandoff = $state<VideoLocalizationHandoffMeta | null>(null);
	let videoLocalizationHandoffPreparing = $state(false);
	...
	const activeVoicePreviewUrl = $derived($store.voiceSource === 'reference_audio' ? (seedAudioFileUrl($store.customVoiceFileId || $store.customVoiceSourceFileId) || $store.customVoicePreviewUrl) : selectedVoicePreviewUrl);
	const customVoiceWaveformUrl = $derived($store.customVoiceSourceFileId
		? `/api/voices/files/${encodeURIComponent($store.customVoiceSourceFileId)}/waveform`
		: '');
	...
	const indexTtsEmotionState = $derived(
		($store.engineUiStateById[INDEX_TTS_ENGINE_ID] as IndexTtsEmotionState | undefined) ??
		createDefaultIndexTtsEmotionState($store.emoAlpha)
	);
	const indexTtsEmotionValidation = $derived(validateIndexTtsEmotionState(indexTtsEmotionState));
	const mainVoiceName = $derived(
		$store.voiceSource === 'reference_audio'
			? ($store.customVoiceFileName || '未选择音色')
			: (selectedVoice?.name || '未选择音色')
	);
	...
	async function restoreRequest(req: GenerateRequest) {
		resetCustomVoice();
		const restoredRequest = req.source === 'video_localization'
			? { ...req, bind_to_video_localization: true }
			: req;
		store.fromRequest(restoredRequest, { preserveVideoLocalizationBinding: req.source === 'video_localization' });
		$store = { ...get(store) };
	...
	function parseVideoLocalizationHandoff(raw: string | null): VideoLocalizationHandoffMeta | null {
		...
		try {
			...
			return {
				...
				subtitle_id: parsed.subtitle_id ?? null,
	...
	function videoLocalizationHandoffFromRequest(req: GenerateRequest): VideoLocalizationHandoffMeta | null {
		if (req.source !== 'video_localization' || !req.project_id) return null;
		const cueId = req.segment_id || req.localized_subtitle_id || req.cue_id;
		...
		return {
			...
			cue_id: cueId,
			subtitle_id: req.localized_subtitle_id ?? null,
	...
	function updateIndexTtsEmotionState(next: IndexTtsEmotionState) { $store.engineUiStateById = { ...$store.engineUiStateById, [INDEX_TTS_ENGINE_ID]: next }; $store.error = ''; }
	function resetCurrentEngineParams() { const keepText = $store.text; const keepVoiceId = $store.voiceId; const keepShowMore = $store.showMoreParams; store.setEngine($store.engineId); if ($store.engineId === INDEX_TTS_ENGINE_ID) updateIndexTtsEmotionState(createDefaultIndexTtsEmotionState()); $store.text = keepText; $store.showMoreParams = keepShowMore; if (usesReferenceVoice) $store.voiceId = keepVoiceId; }
	...
	function syncCustomVoiceTrimMetadata() { const { start, end } = selectedTrimRange(); $store.customVoiceTrimStartMs = Math.round(start * 1000); $store.customVoiceTrimEndMs = Math.round(end * 1000); }
	...
	function updateCustomVoiceTrimRange(startMs: number, endMs: number) {
		const draft = referenceAudioDraftFromLegacyState(get(store), 'generate-custom-reference');
		// History reuse intentionally has no browser File. The managed source path is
		// still a valid non-destructive draft and must invalidate the old processed
		// clip/transcript exactly like a freshly uploaded source.
		draft.source.previewUrl = customVoiceSourcePreviewUrl || draft.source.previewUrl;
		const edited = withReferenceAudioTrim(draft, startMs, endMs);
		const patch = legacyCustomVoicePatchFromDraft(edited);
		setCustomVoicePreviewUrl(patch.customVoicePreviewUrl);
		store.update((state) => ({ ...state, ...patch }));
		stopVoicePreview();
	}
	function customVoiceBusyText() { return customVoiceBusyMode === 'source' ? '正在读取参考音频' : '正在处理选区并识别台词'; }
	function customVoiceStatusText() { if ($store.customVoiceBusy) return customVoiceBusyText(); if ($store.customVoiceConfirmed) return '参考音色与台词已匹配'; if (customVoiceSelectionDirty) return '选区已调整，请重新识别'; if ($store.customVoiceTranscript.trim()) return '可编辑台词，已可使用'; if ($store.customVoiceFileName) return '选择范围后使用选区'; return '点击选择或拖入文件，视频会自动抽取音频'; }
	...
	function resetCustomVoice() { if ($store.customVoicePreviewUrl) revokeObjectUrlIfNeeded($store.customVoicePreviewUrl); if (customVoiceSourcePreviewUrl && customVoiceSourcePreviewUrl !== $store.customVoicePreviewUrl) revokeObjectUrlIfNeeded(customVoiceSourcePreviewUrl); customVoiceOriginalFile = null; $store.customVoiceSelectionDirty = false; customVoiceBusyMode = ''; $store.customVoiceFileName = ''; $store.customVoiceFileId = ''; $store.customVoicePreviewUrl = ''; $store.customVoiceReferenceAudioPath = ''; $store.customVoiceSourceFileId = ''; $store.customVoiceSourceAudioPath = ''; $store.customVoiceSourceDurationMs = null; $store.customVoiceTrimStartMs = null; $store.customVoiceTrimEndMs = null; $store.customVoiceTranscript = ''; $store.customVoiceSrt = ''; $store.customVoiceDurationMs = null; $store.customVoiceSrtSegmentCount = 0; $store.customVoiceTranscriptionId = ''; $store.customVoiceConfirmed = false; $store.customVoiceBusy = false; $store.customVoiceError = ''; $store.customVoiceQualityWarnings = []; resetVoiceRegisterDialog(); stopVoicePreview(); }
	...
	function applyCustomVoiceClipResult(uploaded: Pick<UploadResult, 'file_id' | 'filename' | 'path' | 'quality'>, transcript: TranscriptionRecord, durationMs: number, previewUrl: string) {
		...
		$store.customVoiceSelectionDirty = false;
	}
	...
	async function handleCustomVoiceFile(file: File) {
		...
		try {
			...
			$store.customVoiceSourceDurationMs = Math.round(duration * 1000);
			$store.customVoiceTrimStartMs = 0;
			$store.customVoiceTrimEndMs = Math.round(Math.max(0.1, duration) * 1000);
	...
	async function restoreCustomVoiceReference(req: GenerateRequest) {
		...
		$store.customVoiceError = '';
		$store.customVoiceSelectionDirty = false;
		const restoreEditorRange = (durationMs: number) => {
			const range = normalizeReferenceAudioTrim(durationMs, req.custom_reference_trim_start_ms ?? null, req.custom_reference_trim_end_ms ?? null);
			$store.customVoiceSourceDurationMs = durationMs;
			$store.customVoiceTrimStartMs = range.startMs;
			$store.customVoiceTrimEndMs = range.endMs;
			$store.customVoiceDurationMs = (range.endMs ?? 0) - (range.startMs ?? 0);
		};
		const recordedDurationMs = req.custom_reference_source_duration_ms;
		if (recordedDurationMs && recordedDurationMs > 0) restoreEditorRange(recordedDurationMs);
		try {
			const rangeHeaders = { Range: 'bytes=0-0' };
			const referenceCheck = sourcePath === referencePath
				? null
				: fetch(referenceAudioUrl, { headers: rangeHeaders });
			const response = await fetch(sourceAudioUrl, { headers: rangeHeaders });
			if (!response.ok) {
			...
			if (referenceCheck) {
				...
				void referenceResponse.body?.cancel();
			}
			if (!recordedDurationMs || recordedDurationMs <= 0) {
				restoreEditorRange(Math.round((await loadAudioDuration(sourceAudioUrl)) * 1000));
			}
		} catch (e) {
			customVoiceOriginalFile = null;
	...
	async function previewSelectedVoice() {
		...
		if (!audio || !activeVoicePreviewUrl) return;
		if ($store.voicePreviewPlaying && !audio.paused) {
			audio.pause();
			$store.voicePreviewPlaying = false;
			return;
		}
		const absoluteUrl = new URL(activeVoicePreviewUrl, window.location.href).href;
		if (audio.src !== absoluteUrl) { audio.src = activeVoicePreviewUrl; audio.currentTime = 0; }
		$store.error = '';
		$store.voicePreviewPlaying = true;
		try { await audio.play(); }
		catch (e) { $store.voicePreviewPlaying = false; $store.error = `音色预览播放失败：${(e as Error).message || '请确认参考音频文件可访问'}`; }
	}
	function handleCustomVoicePreviewEnded() { $store.voicePreviewPlaying = false; }
	function handleVoicePreviewPause() { if ($store.voicePreviewAudio?.paused) $store.voicePreviewPlaying = false; }
	function stopVoicePreview() { if ($store.voicePreviewAudio && !$store.voicePreviewAudio.paused) $store.voicePreviewAudio.pause(); $store.voicePreviewPlaying = false; }
	function currentSubmissionVoiceLabel() {
		if (isSeedAudio) {
			return seedAudioState.mode === 'audio' ? '参考声音续写' : seedAudioState.mode === 'image' ? '图片生成音频' : '文本生成音频';
		}
		if ($store.voiceSource === 'reference_audio') return $store.customVoiceFileName || '自定义音色';
		if (selectedVoice?.name) return selectedVoice.name;
		if (doubaoSelectedSpeaker?.label) return doubaoSelectedSpeaker.label;
		const speakerLabel = speakerChoices.find(option => option.value === $store.speakerId)?.label;
		if (speakerLabel) return speakerLabel;
		const mimoVoiceLabel = mimoVoiceOptions.find(option => option.value === $store.mimoVoice)?.label;
		if (isMimoPreset && mimoVoiceLabel) return mimoVoiceLabel;
		if ($store.voiceDesignPrompt.trim()) return '声音设计';
		return '当前引擎默认声音';
	}
	function startSubmissionPreview(inputText: string) {
		const submissionId = crypto.randomUUID();
		submissionPreview = createSubmissionPreview({
			inputText: inputText.trim() || '音频生成任务',
			engineId: $store.engineId,
			engineName: selected?.manifest.display_name ?? $store.engineId,
			engineKind: selected?.manifest.engine_type === 'cloud' ? 'cloud' : 'local',
			voiceLabel: currentSubmissionVoiceLabel()
		}, { submissionId, createdAt: new Date().toISOString() });
		$store.currentPage = 1;
		return submissionId;
	}
	function upsertTask(t: GenerationTask) {
		$store.tasks = upsertTaskPreservingOrder($store.tasks, t);
		submissionPreview = completeSubmissionTask(submissionPreview, t.task_id);
	}
	...
	async function poll(taskId: string) {
		for (let i = 0; i < 900; i++) {
			...
			if (['success', 'failed', 'cancelled'].includes(task.status)) {
				locallySubmittedTaskIds.delete(taskId);
				scheduleTaskPageRefresh(0);
				return;
			}
	...
	async function refreshComposerData() {
		...
		composerDataPromise = (async () => {
			const intentRaw = sessionStorage.getItem(VIDEO_LOCALIZATION_TTS_HANDOFF_INTENT_KEY);
			const handoffIntent = parseVideoLocalizationTtsHandoffIntent(intentRaw);
			const preparedHandoffPromise = handoffIntent
				? Api.previewVideoLocalizationTtsHandoff(
					handoffIntent.project_id,
					handoffIntent.segment_id,
					{
						target_subtitle_ids: handoffIntent.target_subtitle_ids,
						source_cue_ids: handoffIntent.source_cue_ids
					}
				)
				: null;
			videoLocalizationHandoffPreparing = Boolean(preparedHandoffPromise);
			...
			// video-localization binding solely from the complete handoff snapshot.
			videoLocalizationHandoff = reuseRaw || handoffIntent ? parseVideoLocalizationHandoff(handoffRaw) : null;
			...
			if (reuseRaw || preparedHandoffPromise) {
				let restored = false;
				try {
					const request = reuseRaw
						? JSON.parse(reuseRaw) as GenerateRequest
						: await preparedHandoffPromise!;
					...
					$store.error = `参数恢复失败：${(e as Error).message || '参考音频或生成参数已失效，请返回项目重新发送'}`;
				}
				if (restored || handoffIntent) sessionStorage.removeItem(VIDEO_LOCALIZATION_TTS_HANDOFF_INTENT_KEY);
				if (restored) sessionStorage.removeItem(VIDEO_LOCALIZATION_TTS_HANDOFF_REQUEST_KEY);
				if (restored) sessionStorage.removeItem(VIDEO_LOCALIZATION_TTS_HANDOFF_META_KEY);
			}
		})().finally(() => {
			videoLocalizationHandoffPreparing = false;
	...
	async function loadTaskPage(params: TaskPageParams = taskPageParams) {
		...
		try {
			...
			$store.tasks = mergeTaskPageWithPinned(
				response.items,
				$store.tasks,
				locallySubmittedTaskIds
			);
	...
	async function generateSeedAudio() {
		...
		try {
			...
			submissionId = startSubmissionPreview(request.text);
			await tick();
			...
			submissionPreview = attachSubmissionTask(submissionPreview, submissionId, response.task_id);
			...
			submissionPreview = clearSubmissionPreview(submissionPreview, submissionId);
	...
	async function generate() {
		...
		try {
			...
			if (isIndexTTS && indexTtsEmotionState.enabled && !indexTtsEmotionValidation.valid) {
				$store.error = indexTtsEmotionValidation.errors[0] || '请先完成独立情绪参考设置。';
				$store.showMoreParams = true;
				return;
			}
			...
			submissionId = startSubmissionPreview(request.text);
			await tick();
			const plan = await Api.generatePlan({ text: request.text, engine_id: request.engine_id, planner_mode: 'auto', target_format: request.output_format });
			...
			const res = await Api.generate(request);
			submissionPreview = attachSubmissionTask(submissionPreview, submissionId, res.task_id);
	...
	async function reuse(t: GenerationTask) {
		...
		try {
			...
			videoLocalizationHandoff = videoLocalizationHandoffFromRequest(request);
			await restoreRequest(request);
			$store.lastGeneratePlan = null;
			$store.textSegments = [];
			$store.showSplitPreview = false;
	...
	async function retryTask(t: GenerationTask) {
		const confirmation = H.cloudReplayConfirmationMessage(t);
		if (confirmation && !window.confirm(confirmation)) return;
		$store.actionBusyTaskId = t.task_id;
		try {
			const res = await Api.retryTask(t.task_id, Boolean(confirmation));
			$store.currentPage = 1;
	...
	async function retryLongformTask(t: LongformTask) {
		const confirmation = H.longformCloudReplayConfirmationMessage(
			t,
			engineMap
		);
		if (confirmation && !window.confirm(confirmation)) return;
		...
		try {
			const n = await Api.retryLongformFailed(
				t.longform_task_id,
				Boolean(confirmation)
			);
			$store.longformTasks = [
				n,
				...$store.longformTasks.filter(
					(i) => i.longform_task_id !== n.longform_task_id
				)
			];
		} finally {
			$store.actionBusyTaskId = '';
	...
	function taskVerificationPending(t: GenerationTask) { if (t.parameters.source === 'engine_diagnosis' || t.status !== 'success' || !t.result_id || H.taskIsLongformSegment(t) || H.taskIsLongformExport(t) || H.taskUsesVideoLocalizationReview(t) || taskVerificationReport(t) || taskVerificationError(t)) return false; const c = new Date(t.completed_at ?? t.created_at).getTime(); if (!Number.isFinite(c)) return false; return Date.now() - c < 5 * 60 * 1000; }
	...
	function taskCanVerify(t: GenerationTask) { return t.status === 'success' && Boolean(t.result_id) && !H.taskIsLongformSegment(t) && !H.taskIsLongformExport(t) && !H.taskUsesVideoLocalizationReview(t); }
	...
	function videoLocalizationTaskLabel(t: GenerationTask) {
		if (t.parameters?.source !== 'video_localization') return '';
		const target = t.localized_subtitle_id || t.cue_id || t.segment_id;
		const binding = t.bind_to_video_localization ? '已关联' : '仅来源';
		return target ? `视频本土化 · ${binding} · ${target}` : `视频本土化 · ${binding}`;
	}
	...
	function startTaskCardStream(total: number) {
		stopTaskCardStream();
		taskCardRenderLimit = stableTaskCardRenderLimit(taskCardRenderLimit, total);
	...
	onMount(() => {
		...
		return () => {
			...
			stopTaskCardStream();
			stopVoicePreview();
			if ($store.customVoicePreviewUrl.startsWith('blob:')) {
				revokeObjectUrlIfNeeded($store.customVoicePreviewUrl);
				$store.customVoicePreviewUrl = seedAudioFileUrl($store.customVoiceFileId || $store.customVoiceSourceFileId);
			}
	...
	$effect(() => {
		const key = taskIdentityKey(pagedTasks);
...
<main class="page generate-page">
	...
	<div class="workbench"><div class="panel stack compose-panel">
		<div class="row gen-section-head"><div><h2>合成预设</h2><p class="muted">跟随当前引擎{isSeedAudio ? '和模式' : ''}，只显示可用于 {selected?.manifest.display_name ?? $store.engineId}{isSeedAudio ? ` · ${seedAudioState.mode === 'text' ? '文本' : seedAudioState.mode === 'audio' ? '语音' : '图片'}模式` : ''} 的参数组合。</p></div><div class="row preset-tools"><span class="muted">{enginePresets.length} 组</span><button class="btn compact preset-toggle-btn" type="button" aria-expanded={presetStripOpen} data-tooltip={presetStripOpen ? '收起预设面板' : '展开预设面板'} onclick={() => (presetStripOpen = !presetStripOpen)}><ChevronRight size={14} /> {presetStripOpen ? '收起' : '展开'}</button></div></div>
		...
		<div class="param-inline-row">
			...
			<div class="param-actions-inline">
				<button class="btn param-action-btn" type="button" data-tooltip="把当前引擎的参数恢复到默认值。正文和已选参考音色会保留。" onclick={resetCurrentEngineParams}><RotateCcw size={14} /> 重置参数</button>
				{#if hasMoreParams}<button class="btn param-action-btn" type="button" onclick={() => ($store.showMoreParams = !$store.showMoreParams)}><Settings size={14} /> {$store.showMoreParams ? '收起高级' : '更多选项'}</button>{/if}
			</div>
			{/if}
		</div>
		{#if isSeedAudio}
		...
		{/if}
			{#if isSeedAudio && seedEditingSlot && $store.customVoicePreviewUrl}<audio bind:this={$store.voicePreviewAudio} src={$store.customVoicePreviewUrl} preload="none" onended={handleCustomVoicePreviewEnded} onpause={handleVoicePreviewPause}></audio>{/if}
			{#if (usesReferenceVoice && $store.voiceSource === 'reference_audio') || (isSeedAudio && seedEditingSlot !== null)}
				<section class="custom-voice-panel">
					<div class="custom-voice-source-row">
						<input type="file" accept="audio/*,video/*" hidden bind:this={customVoiceFileInput} onchange={(event) => { const file = event.currentTarget.files?.[0]; event.currentTarget.value = ''; if (file) void handleCustomVoiceFile(file); }} />
						<button type="button" class="custom-voice-dropzone" aria-label="点击或拖拽导入参考音频或视频" disabled={$store.customVoiceBusy} onclick={() => customVoiceFileInput?.click()} class:drag-active={customVoiceDragActive} ondragenter={(e) => { e.preventDefault(); customVoiceDragActive = true; }} ondragover={(e) => { e.preventDefault(); customVoiceDragActive = true; }} ondragleave={(e) => { e.preventDefault(); customVoiceDragActive = false; }} ondrop={onCustomVoiceDrop}>
							<CloudUpload size={22} />
							<div>
							<strong>{$store.customVoiceFileName || '点击或拖入参考音频或视频'}</strong>
					...
					{#if customVoiceOriginalFile || $store.customVoiceSourceFileId}
						<ReferenceAudioRangeEditor
							ariaLabel="裁切选区。将鼠标移入或聚焦时间线后，空格播放或暂停；I 设置入点，O 设置出点"
							purposeLabel="自定义音色"
							focusKey={`${$store.requestProjectId}:${$store.requestSegmentId}:${$store.customVoiceSourceFileId}`}
							sourceUrl={customVoiceSourcePreviewUrl}
							waveformUrl={customVoiceWaveformUrl}
			...
			{/if}
		{#if !isSeedAudio && $store.showMoreParams && hasMoreParams}<div class="more-params-panel" class:doubao-tts-params={isDoubao}>
			{#if isDoubaoPreset}<div class="advanced-subhead">朗读控制</div>{/if}
			{#if activeParamKeys.has('speaker_id') && !isDoubaoPreset}
				<div class="field param-field" class:span-wide={hasSearchableSpeakerCatalog} class:field-muted={isQwen3TTS && !qwen3PresetRoute}>
			...
			{#if isDoubao}<div class="advanced-subhead">声音控制与文件</div>{/if}
			...
			{#if isDoubao}<div class="advanced-subhead">交付标记</div>{/if}
		...
		{#if !isSeedAudio}<TextInput
			bind:text={$store.text}
			engineId={$store.engineId}
			ontexttool={(mode: 'clean' | 'numbers' | 'split') => runTextTool(mode)}
			textToolBusy={$store.textToolBusy}
			onGenerate={generate}
			generateBusy={$store.busy}
			associationChecked={$store.requestBindToVideoLocalization}
			associationDetail={videoLocalizationHandoff ? `生成后关联回 ${videoLocalizationHandoff.subtitle_id || videoLocalizationHandoff.cue_id}` : ''}
			associationReturnHref={videoLocalizationHandoff ? `/video-localization?project_id=${encodeURIComponent(videoLocalizationHandoff.project_id)}` : ''}
			onAssociationChange={(checked) => ($store.requestBindToVideoLocalization = checked)}
		/>{/if}
		...
		<div class="result-panel stack section-divider" id="records">
			...
			<div class="row gen-section-head result-headline">
				...
				<div class="records-head-right">
					<div class="records-row-summary" role="status" aria-live="polite">{#if submissionPreview}<span class="badge warn">初始化中</span>{/if}{#if recordsRefreshing}<span class="badge">加载中</span>{/if}{#if taskCardsStreaming}<span class="badge">显示中 {renderedPagedTasks.length}/{pagedTasks.length}</span>{/if}{#if statusCounts.active}<span class="badge">生成中 {queueCounts.processing}</span><span class="badge">等待 {queueCounts.waiting}</span>{/if}{#if $store.selectedTaskIds.length}<span class="badge ok">已选 {$store.selectedTaskIds.length}</span>{/if}</div>
			...
			<div class="records-toolbar">
				<div class="toolbar-control-row">
					<div class="records-filter-inline">
						...
						<select class="engine-filter" bind:value={$store.taskEngineFilter} onchange={() => ($store.currentPage = 1)}>{#each taskEngineOptions as o}<option value={o}>{o === 'all' ? '全部模型' : o}</option>{/each}</select>
						<select class="source-filter" bind:value={$store.taskSourceFilter} onchange={() => ($store.currentPage = 1)}><option value="all">全部来源</option><option value="local">本地</option><option value="cloud">云端</option></select>
						<select class="sort-filter" bind:value={$store.taskSortBy} onchange={() => ($store.currentPage = 1)}><option value="latest">最新</option><option value="oldest">最旧</option><option value="duration_desc">时长↓</option></select>
						<select class="page-size-filter" aria-label="每页显示记录数量" value={$store.pageSizeAuto ? 'auto' : String($store.pageSize)} onchange={(e) => setTaskPageSizePreset((e.currentTarget as HTMLSelectElement).value)}>
			...
			{#if visibleLongformTasks.length}<section class="longform-list"><div class="row section-subhead"><div><h3>长文本任务</h3><p class="muted">进行中的分段生成、校对、重试和合并父任务。</p></div></div>{#each visibleLongformTasks.slice(0, 6) as t}<article class={`longform-card ${t.status}`}><div class="longform-card-head"><div><strong>{H.longformTitle(t)}</strong><p class="muted">{H.longformStatusText(t)}</p></div><div class="row"><span class="badge">{Math.round(t.progress * 100)}%</span>{#if t.export_id}<a class="btn" href={H.longformDownloadUrl(t)}>下载合并音频</a>{/if}{#if t.status === 'failed'}<button class="btn" type="button" onclick={() => retryLongformTask(t)} disabled={$store.actionBusyTaskId === t.longform_task_id}><RotateCcw size={15} /> 重试失败段</button>{/if}<button class="gen-icon-btn mini" type="button" aria-label={longformTaskActionLabel(t)} data-tooltip={longformTaskActionTooltip(t)} onclick={() => handleLongformTaskAction(t)} disabled={$store.actionBusyTaskId === t.longform_task_id}>{#if H.statusIsActive(t.status)}<X size={14} />{:else}<Trash2 size={14} />{/if}</button></div></div><div class="progress-track"><div class="progress-fill" style={`width:${Math.max(3, Math.round(t.progress * 100))}%`}></div></div><div class="longform-segments">{#each t.segments as seg}<div class={`longform-segment ${seg.status}`}><span class="badge">{seg.index}</span><p>{seg.text}</p><span class="badge">{taskStatusLabel(seg.status)}</span>{#if seg.verification}<span class={`badge verify-${seg.verification.status}`}>{H.verificationStatusLabel(seg.verification.status)}</span>{/if}{#if seg.error_message}<span class="muted">{seg.error_message}</span>{/if}</div>{/each}</div>{#if t.error_message}<p class="muted error-line task-error-scroll" use:checkOverflow={t.error_message}>{t.error_message}</p>{/if}</article>{/each}</section>{/if}
			{#if submissionPreview || pagedTasks.length}<section class="result-records-block" class:has-longform-above={visibleLongformTasks.length > 0}><div class="result-grid" bind:this={$store.resultGridEl}>{#if submissionPreview}<article class={`card stack result-card engine-surface submission-preview-card ${submissionPreview.engineKind === 'cloud' ? 'engine-cloud' : 'engine-local'}`} aria-live="polite"><div class="result-head submission-preview-head"><strong class="result-title" title={submissionPreview.inputText}>{submissionPreview.inputText}</strong><span class="badge result-status warn">初始化中</span></div><div class="result-meta result-meta-primary"><span class="badge"><Mic size={11} /> {submissionPreview.voiceLabel}</span><span class="badge result-script-chip"><FileText size={13} /> 台词</span></div><div class="result-meta result-meta-secondary"><span class="badge engine"><Cpu size={11} /> {submissionPreview.engineName}</span><span class="badge badge-kind">{submissionPreview.engineKind === 'cloud' ? '云端' : '本地'}</span><span class="badge meta-pop">{H.formatTime(submissionPreview.createdAt)}</span></div><div class="row result-info"><p class="muted result-subline">正在创建任务记录…</p></div><div class="progress-block submission-preview-progress"><div class="row progress-head"><span class="muted">正在初始化</span><span class="badge">请稍候</span></div><div class="progress-track waiting-track"><div class="progress-fill submission-preview-fill"></div></div><div class="row progress-foot"><span class="muted">任务出现后会自动更新状态</span></div></div></article>{/if}{#each renderedPagedTasks as t (t.task_id)}<article class={`card stack result-card engine-surface ${H.engineKind(t.engine_id, engineMap) === 'cloud' ? 'engine-cloud' : 'engine-local'}${$store.playingResultTaskId === t.task_id && ($store.resultAudioPlaying || resultAudioPendingTaskId === t.task_id) ? ' playing' : ''}`}><div class="result-head"><div class="title-row"><input type="checkbox" checked={$store.selectedTaskIds.includes(t.task_id)} disabled={!H.taskCanDelete(t)} onchange={(e) => toggleTaskSelection(t.task_id, (e.currentTarget as HTMLInputElement).checked)} /><strong class="result-title" title={H.displayTitle(t)}>{H.displayTitle(t)}</strong></div><span class="badge result-status" class:ok={t.status === 'success'} class:fail={t.status === 'failed'} class:warn={t.status === 'cancelled' || H.taskIsActive(t)}>{H.taskStatusPillLabel(t, queueCounts, queueOrderedTasks)}</span></div><div class="result-meta result-meta-primary">{#if t.parameters.source === 'engine_diagnosis'}<span class="badge">引擎试听</span>{/if}{#if videoLocalizationTaskLabel(t)}<span class="badge"><Captions size={11} />{videoLocalizationTaskLabel(t)}</span>{/if}<span class="badge"><Mic size={11} /> {H.voiceBadgeLabel(t, voiceMap, engineMap)}</span><HoverCopyPopover label="台词" title="台词内容" copyText={H.displayTitle(t)}><button type="button" class="badge result-script-chip" aria-label="查看台词内容"><FileText size={13} /> 台词</button></HoverCopyPopover>{#if H.longformResultLabel(t)}<span class="badge longform-result-badge" class:merged={H.taskIsLongformExport(t)} title={H.longformResultTitle(t)}>{H.longformResultLabel(t)}</span>{/if}</div><div class="result-meta result-meta-secondary"><span class="badge engine"><Cpu size={11} /> {engineMap.get(t.engine_id)?.manifest.display_name ?? t.engine_id}</span><span class="badge badge-kind">{H.engineTypeLabel(t.engine_id, engineMap)}</span>{#if t.created_at}<span class="badge meta-pop" data-text={`生成时间：${H.formatTime(t.created_at)}`}>{H.formatTime(t.created_at)}</span>{/if}</div><div class="row result-info"><p class="muted result-subline">{H.taskTimingLine(t)}</p><div class="row result-info-right">{#if t.result_duration_ms}<span class="badge meta-pop" data-text={`音频时长：${H.formatAudioDuration(t.result_duration_ms)}`}>{H.formatAudioDuration(t.result_duration_ms)}</span>{/if}{#if H.taskIsActive(t)}<button class="gen-icon-btn danger" aria-label="取消任务" data-tooltip={taskCancelTooltip(t)} onclick={() => cancelTask(t)} disabled={$store.actionBusyTaskId === t.task_id}><X size={14} /></button>{/if}<HoverCopyPopover label="参数" title="生成参数" copyText={H.taskParameterCopyText(t, engineMap, voiceMap)}><button type="button" class="param-pop compact" aria-label="查看生成参数"><SlidersHorizontal size={13} /></button></HoverCopyPopover></div></div>{#if H.taskIsActive(t)}<div class="progress-block"><div class="row progress-head"><span class="muted">{H.taskStageLabel(t, queueOrderedTasks)}</span><span class="badge">{H.progressLabel(t, queueOrderedTasks)}</span></div><div class="progress-track" class:waiting-track={H.taskIsWaiting(t)}><div class="progress-fill" class:waiting-fill={H.taskIsWaiting(t)} style={`width:${H.taskProgressWidth(t)}%`}></div></div><div class="row progress-foot"><span class="muted">{#if H.taskIsWaiting(t)}已等待 {H.formatSeconds(H.waitingSeconds(t))}{:else}已运行 {H.elapsedLabel(t)}{/if}</span>{#if H.taskEtaLabel(t)}<span class="muted">{H.taskEtaLabel(t)}</span>{/if}</div>{#if H.taskRuntimeHint(t, queueCounts, queueOrderedTasks)}<p class="progress-hint">{H.taskRuntimeHint(t, queueCounts, queueOrderedTasks)}</p>{/if}</div>{/if}{#if t.result_id}{@const downloadName = H.resultDownloadName(t)}<div class="result-audio-row"><WaveformResultPlayer task={t} audioUrl={resultAudioUrl(t)} peaksUrl={`/api/history/${encodeURIComponent(t.result_id)}/waveform`} downloadUrl={resultDownloadUrl(t, downloadName)} durationLabel={t.result_duration_ms ? H.formatAudioDuration(t.result_duration_ms) : '播放结果'} isPlaying={$store.playingResultTaskId === t.task_id && ($store.resultAudioPlaying || resultAudioPendingTaskId === t.task_id)} isPending={resultAudioPendingTaskId === t.task_id} currentTime={$store.playingResultTaskId === t.task_id ? resultAudioCurrentTime : 0} onPlay={() => toggleResultPlayback(t)} onStop={() => toggleResultPlayback(t)} onSeek={(task, timeSeconds) => seekResultPlayback(task, timeSeconds)} /></div>{/if}{#if t.error_message}<p class="muted error-line task-error-scroll" use:checkOverflow={H.knownErrorMessage(t.error_message)}>{H.knownErrorMessage(t.error_message)}</p>{/if}<div class="result-footer"><div class="result-footer-status">{#if taskVerificationReport(t)}{@const r = taskVerificationReport(t)}<div class="verification-line {r.status}"><span class="dot"></span>{H.verificationStatusLabel(r.status)}{#if r.status !== 'skipped'}<span class="coverage">覆盖率 {Math.round(r.coverage * 100)}%</span>{/if}</div>{:else if taskVerificationPending(t)}<p class="muted verification-pending-line">自动校对中…</p>{:else if taskVerificationError(t)}<p class="muted error-line">{H.knownErrorMessage(taskVerificationError(t))}</p>{:else if taskCanVerify(t)}<button class="verification-action" type="button" onclick={() => verifyTask(t)} disabled={$store.verificationBusyTaskId === t.task_id}>{$store.verificationBusyTaskId === t.task_id ? '校对中…' : '尚未校对 · 立即校对'}</button>{/if}</div><div class="row result-card-actions">{#if t.status === 'failed'}<button class="gen-icon-btn" aria-label="重试任务" data-tooltip="使用原参数重新尝试生成" onclick={() => retryTask(t)} disabled={$store.actionBusyTaskId === t.task_id}><RotateCcw size={15} /></button>{/if}{#if H.taskCanDelete(t)}<button class="gen-icon-btn" aria-label="复用参数" data-tooltip="把这条记录的文本和参数带回上方生成区" onclick={() => reuse(t)} disabled={$store.actionBusyTaskId === t.task_id}><Repeat size={15} /></button>{/if}{#if !H.taskIsActive(t)}<button class="gen-icon-btn danger" aria-label="删除记录" data-tooltip="删除这条任务记录" onclick={() => deleteTaskRecord(t)} disabled={$store.actionBusyTaskId === t.task_id}><Trash2 size={15} /></button>{/if}</div></div></article>{/each}</div>{#if pageCount > 1}<div bind:this={recordsBottomPagerEl} class="pagination-bar"><button class="btn" onclick={() => $store.currentPage = 1} disabled={$store.currentPage <= 1}><ChevronsLeft size={15} /> 首页</button><button class="btn" onclick={() => taskPageJump(-1)} disabled={$store.currentPage <= 1}><ChevronLeft size={15} /> 上一页</button><span class="muted">第 {$store.currentPage} / {pageCount} 页</span><div class="page-jump"><span class="muted">跳至</span><input type="number" min="1" max={pageCount} bind:value={$store.pageJumpInput} onkeydown={(e) => e.key === 'Enter' && jumpToPage()} /><span class="muted">页</span></div><button class="btn" onclick={() => $store.currentPage = pageCount} disabled={$store.currentPage >= pageCount}>下一页 <ChevronRight size={15} /></button><button class="btn" onclick={() => $store.currentPage = pageCount} disabled={$store.currentPage >= pageCount}>尾页 <ChevronsRight size={15} /></button></div>{/if}</section>{:else}<div class="empty">{#if recordsInitialized}当前筛选下没有任务记录。{:else}正在加载任务记录...{/if}</div>{/if}
			</section>
			</div>
		</div>
	</div>
