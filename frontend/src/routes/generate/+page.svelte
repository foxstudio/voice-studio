...
import type { TaskDateFilter, TaskSortBy, TaskSourceFilter, TaskStatusTab } from './helpers';
	...
	const isEmotiVoice = $derived($store.engineId === 'emotivoice'); const isF5 = $derived($store.engineId === 'f5-tts'); const isConfucius4 = $derived($store.engineId === 'confucius4-mlx-int8');
	...
	const usesReferenceVoice = $derived(isIndexTTS || isOmniVoice || isConfucius4 || isMimoClone || isF5 || isCosyVoiceZeroShot);
	...
	<div class="workbench"><div class="panel stack compose-panel">
		...
		<div class="param-inline-row">
			...
			<div class="param-actions-inline">
				...
				<button class="btn param-action-btn param-inline-more" type="button" onclick={() => ($store.showMoreParams = !$store.showMoreParams)}><Settings size={14} /> {$store.showMoreParams ? '收起高级' : '更多选项'}</button>
			</div>
		</div>
		...
		{#if $store.showMoreParams}<div class="more-params-panel">
			...
			{#if activeParamKeys.has('prompt')}<div class="field param-field" class:span-full={!isEmotiVoice}><label class="param-label" for="vprompt">情绪提示</label><div class="param-control"><select id="vprompt" bind:value={$store.voicePrompt}>{#each promptOptions as o}<option value={o.value}>{o.label}</option>{/each}</select></div></div>{/if}
			...
			{#if supportsEmotion}<div class="field param-field"><label class="param-label" for="emo">情绪</label><div class="param-control"><select id="emo" bind:value={$store.emotion}><option value="">跟随参考音色</option><option value="calm">自然</option><option value="happy">高兴</option><option value="sad">悲伤</option><option value="angry">愤怒</option><option value="afraid">恐惧</option><option value="disgusted">反感</option><option value="melancholic">低落</option><option value="surprised">惊讶</option></select></div></div>{#if isIndexTTS && !followsReferenceEmotion}<Field label="情绪强度" tooltip="情感强度，0.0=无情感，1.0=最大情感表达"><Slider value={$store.emoAlpha} min={0} max={1} step={0.05} onChange={(v: number) => $store.emoAlpha = v} /></Field>{/if}{/if}
			...
			{#if selected}<ParameterPanel autoExpand={true} parameterSchema={selected.manifest.parameter_schema.filter(p => p.level === 'advanced' || p.level === 'developer')} values={{ temperature: $store.temperature, top_p: $store.topP, top_k: $store.topK, seed: $store.seed, max_text_tokens_per_segment: $store.maxTextTokensPerSegment, interval_silence: $store.intervalSilence, diffusion_steps: $store.diffusionSteps, cfg_rate: $store.cfgRate, guidance_scale: $store.guidanceScale, duration: $store.duration, audio_chunk_duration: $store.audioChunkDuration, audio_chunk_threshold: $store.audioChunkThreshold, max_mel_tokens: $store.maxMelTokens, repetition_penalty: $store.repetitionPenalty, nfe_step: $store.nfeStep, cfg_strength: $store.cfgStrength, target_rms: $store.targetRms, cross_fade_duration: $store.crossFadeDuration, sway_sampling_coef: $store.swaySamplingCoef, fix_duration: $store.fixDuration, remove_silence: $store.removeSilence, optimize_text_preview: $store.optimizeTextPreview }} onChange={(k, v) => { const st: Record<string, (x: unknown) => void> = { temperature: (x: unknown) => $store.temperature = x as number, top_p: (x: unknown) => $store.topP = x as number, top_k: (x: unknown) => $store.topK = x as number, seed: (x: unknown) => $store.seed = x === '' || x === null || x === undefined ? null : Number(x), max_text_tokens_per_segment: (x: unknown) => $store.maxTextTokensPerSegment = x as number, interval_silence: (x: unknown) => $store.intervalSilence = x as number, diffusion_steps: (x: unknown) => $store.diffusionSteps = x as number, cfg_rate: (x: unknown) => $store.cfgRate = x as number, guidance_scale: (x: unknown) => $store.guidanceScale = x as number, duration: (x: unknown) => $store.duration = x as number, audio_chunk_duration: (x: unknown) => $store.audioChunkDuration = x as number, audio_chunk_threshold: (x: unknown) => $store.audioChunkThreshold = x as number, max_mel_tokens: (x: unknown) => $store.maxMelTokens = x as number, repetition_penalty: (x: unknown) => $store.repetitionPenalty = x as number, nfe_step: (x: unknown) => $store.nfeStep = x as number, cfg_strength: (x: unknown) => $store.cfgStrength = x as number, target_rms: (x: unknown) => $store.targetRms = x as number, cross_fade_duration: (x: unknown) => $store.crossFadeDuration = x as number, sway_sampling_coef: (x: unknown) => $store.swaySamplingCoef = x as number, fix_duration: (x: unknown) => $store.fixDuration = x as number, remove_silence: (x: unknown) => $store.removeSilence = x as boolean, optimize_text_preview: (x: unknown) => $store.optimizeTextPreview = x as boolean }; st[k]?.(v); }} />{/if}
		</div>{/if}
				...
				<div class="gen-modal-backdrop">
					<div class="voice-register-dialog" role="dialog" aria-modal="true">
						...
						<div class="voice-register-grid">
							...
							<label class="field"><span>推荐引擎</span><select bind:value={voiceRegisterEngine}><option value="indextts-v2">IndexTTS v2</option><option value="omnivoice">OmniVoice</option><option value="confucius4-mlx-int8">Confucius4-TTS MLX int8</option><option value="f5-tts">F5-TTS</option><option value="cosyvoice-zero-shot">CosyVoice Zero-Shot</option><option value="mimo-v2.5-tts-voiceclone">MiMo VoiceClone</option></select></label>
