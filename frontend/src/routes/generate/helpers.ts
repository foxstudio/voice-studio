...
export function taskUsesVideoLocalizationReview(t: GenerationTask) {
	return t.parameters?.source === 'video_localization' && Boolean(t.bind_to_video_localization);
}

export function cloudReplayConfirmationMessage(t: GenerationTask) {
	if (!t.provider_state_uncertain) return '';
	return '这次云端生成的结果无法确认，重新提交可能再次计费。请先核对云端状态；确认仍要重新生成吗？';
}

export function longformCloudReplayConfirmationMessage(
	t: LongformTask,
	engineMap: Map<string, EngineDetail>
) {
	if (engineKind(t.engine_id, engineMap) !== 'cloud') return '';
	return '这条长文本包含失败的云端分段，重试可能再次计费。请先核对云端状态；确认仍要重试失败分段吗？';
}
...
export function requestFromTask(task: GenerationTask): GenerateRequest {
	...
	return {
		...
		source: params.source ?? (task.project_id && (task.localized_subtitle_id || task.cue_id || task.bind_to_video_localization) ? 'video_localization' : undefined),
		project_id: params.project_id ?? task.project_id,
		segment_id: params.segment_id ?? task.segment_id,
		localized_subtitle_id: params.localized_subtitle_id ?? task.localized_subtitle_id,
		cue_id: params.cue_id ?? task.cue_id,
		bind_to_video_localization: params.bind_to_video_localization ?? task.bind_to_video_localization,
...
export function progressLabel(task: GenerationTask, _queueOrderedTasks: GenerationTask[]) {
...
export function taskStatusPillLabel(task: GenerationTask, queueCounts: { processing: number; waiting: number }, _queueOrderedTasks: GenerationTask[]) {
...
export function taskStageLabel(task: GenerationTask, _queueOrderedTasks: GenerationTask[]) {
	if (taskIsWaiting(task)) {
		return '等待后台安排';
	}
...
export function queueSummaryText(queueCounts: { processing: number; waiting: number }, queueOrderedTasks: GenerationTask[], engineMap: Map<string, EngineDetail>) {
	...
	return `等待 ${queueCounts.waiting} 条任务，后台按项目与优先级调度。`;
}
...
export function taskRuntimeHint(task: GenerationTask, queueCounts: { processing: number; waiting: number }, _queueOrderedTasks: GenerationTask[]) {
	if (taskIsWaiting(task)) {
		return queueCounts.processing
			? '按项目与优先级调度，当前任务结束后安排。'
			: '按项目与优先级调度，等待后台安排。';
	}
