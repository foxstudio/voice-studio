...
export type AudioFrameIntervalInput = AudioFrameInterval & {
	frameRate: number;
...
export function minimumFrameDurationMs(frameRate: number) {
	return Math.max(1, Math.round(frameDurationMs(frameRate)));
}
...
export function timelineRangeWidthPercent(
	startMs: number,
	endMs: number,
	durationMs: number
) {
	if (!Number.isFinite(durationMs) || durationMs <= 0) return 0;
	const start = Math.max(0, Number.isFinite(startMs) ? startMs : 0);
	const end = Math.max(start, Number.isFinite(endMs) ? endMs : start);
	const left = Math.max(0, Math.min(100, (start / durationMs) * 100));
	return Math.max(
		0,
		Math.min(100 - left, ((end - start) / durationMs) * 100)
	);
}
...
export function editFrameInterval({
	...
	// A click or sub-frame pointer jitter is not a timing edit.
	if (deltaFrames === 0) return { startMs, endMs };
	...
	return {
		startMs: mode === 'trim-end' ? startMs : frameTimeMs(nextStartFrame, frameRate),
		endMs: mode === 'trim-start' ? endMs : frameTimeMs(nextEndFrame, frameRate)
	};
}
...
export function editAudioFrameInterval({
	...
	const safeSourceDuration = snapTimeToFrame(
		Math.max(normalized.sourceEndMs, sourceDurationMs),
		frameRate,
		'nearest',
		normalized.sourceEndMs
	);
	const minStartMs = mode === 'trim-start'
		? Math.max(0, normalized.startMs - normalized.sourceStartMs)
		: 0;
	const maxEndMs = mode === 'trim-end'
		? Math.min(
				timelineDurationMs,
				normalized.startMs + Math.max(0, safeSourceDuration - normalized.sourceStartMs)
			)
		: timelineDurationMs;
	const interval = editFrameInterval({
		...
		minStartMs,
		maxEndMs,
		minDurationMs: frameDurationMs(frameRate)
	});
	let nextSourceStart = normalized.sourceStartMs;
	let nextSourceEnd = normalized.sourceEndMs;
	if (mode === 'trim-start') {
		nextSourceStart = normalized.sourceStartMs + interval.startMs - normalized.startMs;
	}
	if (mode === 'trim-end') {
		nextSourceEnd = normalized.sourceStartMs + interval.endMs - normalized.startMs;
	}
...
export function normalizeAudioFrameInterval({
	startMs,
	endMs,
	sourceStartMs,
	sourceEndMs,
	frameRate,
	timelineDurationMs
}: AudioFrameIntervalInput): AudioFrameInterval {
	const timelineStartFrame = frameIndexAtTime(startMs, frameRate, 'nearest');
	const maximumTimelineEndFrame = Math.max(
		timelineStartFrame + 1,
		frameIndexAtTime(timelineDurationMs, frameRate, 'floor')
	);
	const timelineEndFrame = clampInteger(
		Math.max(timelineStartFrame + 1, frameIndexAtTime(endMs, frameRate, 'nearest')),
		timelineStartFrame + 1,
		maximumTimelineEndFrame
	);
	const sourceStartFrame = frameIndexAtTime(sourceStartMs, frameRate, 'nearest');
	const sourceEndFrame = Math.max(
		sourceStartFrame + 1,
		frameIndexAtTime(sourceEndMs, frameRate, 'nearest')
	);
	const playableFrameCount = Math.max(
		1,
		Math.min(
			timelineEndFrame - timelineStartFrame,
			sourceEndFrame - sourceStartFrame,
			maximumTimelineEndFrame - timelineStartFrame
		)
	);

	return {
		startMs: frameTimeMs(timelineStartFrame, frameRate),
		endMs: frameTimeMs(timelineStartFrame + playableFrameCount, frameRate),
		sourceStartMs: frameTimeMs(sourceStartFrame, frameRate),
		sourceEndMs: frameTimeMs(sourceStartFrame + playableFrameCount, frameRate)
	};
}
