<script module lang="ts">
	export function nextWaveformActivation(
		activated: boolean,
		entries: ReadonlyArray<{ isIntersecting: boolean }>
	) {
		return activated || entries.some((entry) => entry.isIntersecting);
	}
</script>
...
<script lang="ts">
	...
	import { waveformBarCount, waveformBars } from './subtitle-workbench';
	...
	let loadToken = 0;
	let activeAudioUrl = '';
	let waveformActivated = $state(false);
	let audioActivated = $state(false);
	let trackWidth = $state(0);

	const displayBars = $derived(waveformBars(peaks, waveformBarCount(trackWidth - 10)));
	...
	onMount(() => {
		...
		const observer = new IntersectionObserver((entries) => {
			waveformActivated = nextWaveformActivation(waveformActivated, entries);
			if (!waveformActivated) return;
			observer.disconnect();
...
<div bind:this={shellEl} class="audio-waveform" class:playing aria-label={label}>
	{#if audioActivated}
		<audio
			bind:this={audioEl}
			preload="metadata"
	...
	<button class="wave-button" type="button" aria-label={playing ? `暂停${label}` : audioPending ? `正在加载${label}` : `播放${label}`} aria-busy={loading || audioPending} data-tooltip={playing ? `暂停${label}` : `播放${label}`} onclick={togglePlayback}>
		{#if loading || audioPending}<CommandSpinner size={13} />{:else if playing}<Pause size={13} />{:else}<Play size={13} />{/if}
	</button>
	<div
		class="wave-track"
	...
	>
		<div class="wave-bars wave-bars-base" aria-hidden="true">
			{#each displayBars as height}
				<span style={`--bar-height:${height * 100}%`}></span>
			{/each}
		</div>
