<script lang="ts">
	import { AlertTriangle, Check, Clipboard, Info, PanelRightOpen, X } from 'lucide-svelte';
...
<div class="activity-slot" class:active={hasMessage || tasks.length > 0}>
	{#if hasMessage}
		<div class:error={displayedKind === 'error'} class:success={displayedKind === 'success'} class:running={displayedKind === 'running'} class="activity-message" role={displayedKind === 'error' ? 'alert' : 'status'} aria-live="polite">
			...
			<span class="activity-summary"><HoverMarqueeText text={displayedSummary} /></span>
				{#if showDetails}
	...
	{:else if tasks.length}
		<div class="activity-message running" role="status" aria-live="polite">
			<span class="activity-icon" aria-hidden="true"><CommandSpinner size={13} /></span>
			<span class="activity-summary"><HoverMarqueeText text={taskSummary.text} /></span>
