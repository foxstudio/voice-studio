...
import { minimumFrameDurationMs } from './frame-timeline';

export const MIN_AUDIO_CLIP_DURATION_MS = minimumFrameDurationMs(30);
...
export function splitTimelineAudioClip(
	...
	clipId: string,
	splitMs: number,
	options: { nextClipId?: string; nextSubtitleId?: string; minDurationMs?: number } = {}
) {
	const clip = clips.find((item) => item.clip_id === clipId);
	if (!clip) return null;
	const minDurationMs = Math.max(1, Math.round(options.minDurationMs ?? MIN_AUDIO_CLIP_DURATION_MS));
	const startMs = Math.max(0, Math.round(clip.start_ms ?? 0));
	const endMs = Math.max(startMs + minDurationMs, Math.round(clip.end_ms ?? startMs + 1800));
	const cutMs = Math.round(splitMs);
	if (cutMs < startMs + minDurationMs || cutMs > endMs - minDurationMs) return null;
