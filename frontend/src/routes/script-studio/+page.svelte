<script lang="ts">
	...
	async function refresh() {
		const [nextProjects, nextVoices, nextEngines] = await Promise.all([
		...
		voices = nextVoices;
		engines = nextEngines;
		const preferredId = currentProjectId || current?.project_id || '';
		currentProjectId = projects.some((project) => project.project_id === preferredId)
			? preferredId
			: projects[0]?.project_id ?? '';
		current = currentProjectId ? await Api.project(currentProjectId) : null;
	}
	$effect(() => { refresh(); });
	$effect(() => {
	...
	async function createProject() {
		...
		currentProjectId = current.project_id;
		await refresh();
	}
...
<main class="page">
	...
	<div class="workbench">
		<section class="panel stack">
			<div class="toolbar">
				<select bind:value={currentProjectId} onchange={selectProject}>
					{#each projects as p}<option value={p.project_id}>{p.name}</option>{/each}
				</select>
