<script lang="ts">
	import { AudioLines, ChevronsLeft, ChevronsRight, Film, Library, Settings, SlidersHorizontal } from 'lucide-svelte';
	import { page } from '$app/state';

	let { collapsed = false, onToggle = () => {}, onNavClick = () => {} }: { collapsed?: boolean; onToggle?: () => void; onNavClick?: () => void } = $props();

	const items = [
		{ href: '/engine-hub', label: '引擎与模型', icon: SlidersHorizontal },
