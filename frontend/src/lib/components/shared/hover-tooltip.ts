...
let popoverManager: HoverPopoverManager | null = null;

class HoverPopoverManager {
	private readonly layer: HTMLDivElement;
	private readonly content = new WeakMap<HTMLElement, HoverPopoverContent>();
	private readonly session: TooltipSession<HTMLElement>;
	private activeTarget: HTMLElement | null = null;
	private fadeTimer: TimerHandle | null = null;
	...
	constructor() {
		this.layer = document.createElement('div');
		this.layer.id = 'shared-hover-popover';
		this.layer.setAttribute('role', 'tooltip');
		Object.assign(this.layer.style, {
			position: 'fixed',
			zIndex: '10020',
	...
	register(node: HTMLElement, value: HoverPopoverContent) {
		this.update(node, value);
		const enter = () => this.session.enterTarget(node);
		const leave = () => this.session.leaveTarget(node);
	...
	update(node: HTMLElement, value: HoverPopoverContent) {
		this.content.set(node, value);
		if (this.session.isActive(node)) this.show(node);
	}

	private show(target: HTMLElement) {
		const content = this.content.get(target);
		if (!content) return;
		if (this.fadeTimer !== null) clearTimeout(this.fadeTimer);
		this.activeTarget?.removeAttribute('aria-describedby');
		this.activeTarget = target;
	...
	private render(content: HoverPopoverContent) {
		this.layer.replaceChildren();
		if (content.title) {
			const title = document.createElement('strong');
			title.textContent = content.title;
			Object.assign(title.style, { display: 'block', marginBottom: '7px', color: '#dce6e9', fontSize: '11px' });
			this.layer.appendChild(title);
		}
		if (content.rows?.length) {
			const list = document.createElement('dl');
			Object.assign(list.style, { display: 'grid', gap: '0', margin: '0' });
			for (const row of content.rows) {
				const item = document.createElement('div');
				Object.assign(item.style, {
					display: 'grid',
					gridTemplateColumns: 'minmax(112px, 0.85fr) minmax(0, 1.15fr)',
					gap: '9px',
					padding: '7px 0',
				...
				Object.assign(label.style, { margin: '0', color: '#809097', fontSize: '10px', lineHeight: '1.45', overflowWrap: 'anywhere' });
				const value = document.createElement('dd');
				value.textContent = row.value;
				Object.assign(value.style, { margin: '0', color: '#c5d1d5', fontSize: '10px', lineHeight: '1.45', whiteSpace: 'pre-wrap', overflowWrap: 'anywhere' });
		...
		empty.textContent = content.emptyText || '没有详细信息';
		Object.assign(empty.style, { color: '#809097', fontSize: '10px' });
		this.layer.appendChild(empty);
	}
	...
	private reposition = () => {
		if (!this.activeTarget || this.layer.style.display === 'none') return;
		const position = computeTooltipPosition(
			this.activeTarget.getBoundingClientRect(),
			{ width: this.layer.offsetWidth, height: this.layer.offsetHeight },
			{ width: window.innerWidth, height: window.innerHeight }
		);
		this.layer.dataset.placement = position.placement;
...
function activePopoverManager() {
	if (!popoverManager) popoverManager = new HoverPopoverManager();
	return popoverManager;
}

export const hoverPopover: Action<HTMLElement, HoverPopoverContent> = (node, value) => {
	if (typeof document === 'undefined') return {};
	const activeManager = activePopoverManager();
	const unregister = activeManager.register(node, value);
	return {
		update(nextValue) {
			activeManager.update(node, nextValue);
