...
export type GenerateStoreState = {
	...
	requestCueId: string;
	requestTimelineClipId: string;
	requestBindToVideoLocalization: boolean;
	requestVideoLocalizationWorkflowId: string;
	requestVideoLocalizationStartMs: number | null;
	requestVideoLocalizationEndMs: number | null;
	requestVideoLocalizationTargetSubtitleIds: string[];
	requestVideoLocalizationSourceCueIds: string[];
	...
	customVoiceSelectionDirty: boolean;
...
function createInitialState(): GenerateStoreState {
	return {
		...
		requestCueId: '',
		requestTimelineClipId: '',
		requestBindToVideoLocalization: false,
		requestVideoLocalizationWorkflowId: '',
		requestVideoLocalizationStartMs: null,
		requestVideoLocalizationEndMs: null,
		requestVideoLocalizationTargetSubtitleIds: [],
		requestVideoLocalizationSourceCueIds: [],
...
const LANGUAGE_ALIASES: Record<string, string> = {
	zh: 'zh', 'zh-cn': 'zh', cn: 'zh', chinese: 'zh', '中文': 'zh',
...
function compatibleEngineLanguage(state: GenerateStoreState, engineId: string, currentLanguage: string) {
	const engine = getSelectedEngine(state, engineId);
	if (!engine || !currentLanguage.trim()) return null;
	const languageParam = engine.manifest.parameter_schema.find((param) => param.key === 'language');
	const declared = languageParam?.options.length
		? languageParam.options.map((option) => String(option.value))
		: engine.manifest.supported_languages.map(String);
	if (!declared.length) return currentLanguage;
	const exact = declared.find((value) => value.toLowerCase() === currentLanguage.trim().toLowerCase());
	if (exact) return exact;
	const canonical = canonicalLanguage(currentLanguage);
	return declared.find((value) => canonicalLanguage(value) === canonical) ?? null;
}
...
function getEngineDefaults(state: GenerateStoreState, engineId: string) {
	...
	return {
		...
		emoAlpha: INDEX_TTS_DEFAULTS.emoAlpha,
		speed: Number(parameterDefault('speed', INDEX_TTS_DEFAULTS.speed)),
		...
		maxTextTokensPerSegment: Number(parameterDefault('max_text_tokens_per_segment', INDEX_TTS_DEFAULTS.maxTextTokensPerSegment)),
		intervalSilence: Number(parameterDefault('interval_silence', INDEX_TTS_DEFAULTS.intervalSilence)),
		...
		guidanceScale: Number(parameterDefault('guidance_scale', OMNIVOICE_DEFAULTS.guidanceScale)),
		duration: Number(parameterDefault('duration', OMNIVOICE_DEFAULTS.duration)),
		audioChunkDuration: Number(parameterDefault('audio_chunk_duration', OMNIVOICE_DEFAULTS.audioChunkDuration)),
		audioChunkThreshold: Number(parameterDefault('audio_chunk_threshold', OMNIVOICE_DEFAULTS.audioChunkThreshold)),
		outputFormat: INDEX_TTS_DEFAULTS.outputFormat,
		...
		nfeStep: Number(parameterDefault('nfe_step', F5_DEFAULTS.nfeStep)),
		cfgStrength: Number(parameterDefault('cfg_strength', F5_DEFAULTS.cfgStrength)),
		targetRms: Number(parameterDefault('target_rms', F5_DEFAULTS.targetRms)),
		crossFadeDuration: Number(parameterDefault('cross_fade_duration', F5_DEFAULTS.crossFadeDuration)),
		...
		removeSilence: Boolean(parameterDefault('remove_silence', F5_DEFAULTS.removeSilence)),
		maxMelTokens: Number(parameterDefault('max_mel_tokens', INDEX_TTS_DEFAULTS.maxMelTokens)),
...
function createRequest(state: GenerateStoreState): GenerateRequest {
	...
	return {
		...
		timeline_clip_id: state.requestTimelineClipId || null,
		bind_to_video_localization:
			state.requestSource === 'video_localization' && state.requestBindToVideoLocalization,
		video_localization_workflow_id: state.requestVideoLocalizationWorkflowId || null,
		video_localization_start_ms: state.requestVideoLocalizationStartMs,
		video_localization_end_ms: state.requestVideoLocalizationEndMs,
		video_localization_target_subtitle_ids: [...state.requestVideoLocalizationTargetSubtitleIds],
		video_localization_source_cue_ids: [...state.requestVideoLocalizationSourceCueIds],
		...
		cfg_scale: isQwen3TTS ? undefined : state.cfgScale,
		ddpm_steps: isQwen3TTS ? undefined : state.ddpmSteps,
...
function applyRequest(
	...
	return {
		...
		requestTimelineClipId: req.timeline_clip_id ?? '',
		requestBindToVideoLocalization:
			options.preserveVideoLocalizationBinding !== false && Boolean(req.bind_to_video_localization),
		requestVideoLocalizationWorkflowId: req.video_localization_workflow_id ?? '',
		requestVideoLocalizationStartMs: req.video_localization_start_ms ?? null,
		requestVideoLocalizationEndMs: req.video_localization_end_ms ?? null,
		requestVideoLocalizationTargetSubtitleIds: [...(req.video_localization_target_subtitle_ids ?? [])],
		requestVideoLocalizationSourceCueIds: [...(req.video_localization_source_cue_ids ?? [])],
...
export function createGenerateStore() {
	...
	return {
		...
		setEngine(engineId: string) {
			store.update((state) => {
				const defaults = getEngineDefaults(state, engineId);
				const preservedLanguage = state.engineId === engineId
					? null
					: compatibleEngineLanguage(state, engineId, state.language);
				return {
					...state,
					...defaults,
