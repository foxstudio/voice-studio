# Source: MLX adaptation aligned to official IndexTTS 2.0 `indextts/infer_v2.py`
# at ref 830f6f8f94a51fea23ab1d639027a86200075a4e. Modified by MLX-IndexTTS,
# then modified by Voice Studio for its maintained runtime. Applicable terms and
# the required distribution notice are in THIRD_PARTY_NOTICES.md.
...
def _resolve_preprocessing_artifact(
    model_dirs: tuple[Path, ...],
    artifact: IndexTTSPreprocessingArtifact,
) -> Path:
    """Resolve a preprocessing dependency without performing network I/O."""
    unique_roots = tuple(dict.fromkeys(Path(root) for root in model_dirs))
    for root in unique_roots:
        candidate = root / artifact.managed_relative_path
    ...
    expected = unique_roots[0] / artifact.managed_relative_path
    raise FileNotFoundError(
        f"Missing offline model dependency {artifact.repo_id}/{artifact.filename} "
        f"at revision {artifact.revision}. "
        f"Place it at {expected} or download it into the Hugging Face cache before inference. "
        "Voice Studio does not download model files while generating audio."
    )


def _resolve_preprocessing_bundle(
    model_dirs: tuple[Path, ...],
    artifacts: tuple[IndexTTSPreprocessingArtifact, ...],
) -> Path:
    """Resolve a group that must come from one revision and one directory."""
    unique_roots = tuple(dict.fromkeys(Path(root) for root in model_dirs))
    for root in unique_roots:
        paths = [root / artifact.managed_relative_path for artifact in artifacts]
    ...
    try:
        ...
        cached_paths = []
    if len(cached_paths) == len(artifacts):
        parents = {path.parent for path in cached_paths}
        if len(parents) == 1:
            return cached_paths[0].parent

    expected = unique_roots[0] / artifacts[0].managed_relative_path
    raise FileNotFoundError(
        f"Missing complete offline model bundle {artifacts[0].repo_id} at "
        f"revision {artifacts[0].revision}. Place all required files beside "
        f"{expected}. Voice Studio does not combine partial model revisions."
    )
...
class IndexTTSv2:
    ...
    def _init_semantic_codec(self):
        ...
        try:
            ...
            ckpt_path = _resolve_preprocessing_artifact(
            ...
            raise RuntimeError(
                "IndexTTS v2 semantic codec weights could not be loaded; "
                "WAV reference preprocessing cannot continue. Install or repair "
                "amphion/MaskGCT semantic_codec/model.safetensors. Runtime downloads are disabled."
            ) from e
    ...
    def _init_pytorch_modules(self):
        ...
        try:
            w2v_model_dir = _resolve_preprocessing_bundle(
                (self.mlx_model_dir, self.model_dir),
                INDEXTTS_W2V_BERT_ARTIFACTS,
            )
            self.semantic_model, self.semantic_mean, self.semantic_std = build_semantic_model(
                path_=str(w2v_stat_path),
                model_name_or_path=str(w2v_model_dir),
            )
            self.semantic_model = self.semantic_model.to(self.device)
            self.semantic_mean = self.semantic_mean.to(self.device)
            self.semantic_std = self.semantic_std.to(self.device)
            self.extract_features = AutoFeatureExtractor.from_pretrained(
                str(w2v_model_dir),
            ...
            self.semantic_model = None
            raise RuntimeError(
                "IndexTTS v2 W2V-BERT files could not be loaded; WAV reference "
                "preprocessing cannot continue. Install or repair facebook/w2v-bert-2.0. "
        ...
        try:
            campplus_path = _resolve_preprocessing_artifact(
                (self.mlx_model_dir, self.model_dir),
            ...
            self.campplus = None
            raise RuntimeError(
                "IndexTTS v2 CAMPPlus weights could not be loaded; "
                "WAV reference preprocessing cannot continue. Install or repair "
                "funasr/campplus campplus_cn_common.bin. Runtime downloads are disabled."
            ) from e
