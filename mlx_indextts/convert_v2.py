...
Usage (via CLI, auto-detects version):
    mlx-indextts convert --model-dir ~/Downloads/IndexTTS-2 -o ~/VoiceStudio/models/mlx-indexTTS-2.0
"""
...
from mlx_indextts.model_artifacts import (
    INDEXTTS_BIGVGAN_FILENAME,
    INDEXTTS_BIGVGAN_REPO_ID,
    INDEXTTS_BIGVGAN_REVISION,
...
def export_vq2emb_weights(cfg) -> Dict[str, mx.array]:
    ...
    artifact = INDEXTTS_WAV_PREPROCESSING_BY_ID["maskgct-semantic-codec"]
...
def install_wav_preprocessing_artifacts(
    output_dir: Path,
    *,
    downloader=None,
) -> list[Path]:
    """Download pinned WAV-cloning assets into the converted model directory."""
    ...
    for artifact in INDEXTTS_WAV_PREPROCESSING_ARTIFACTS:
        ...
        destination = output_dir / artifact.managed_relative_path
...
def convert_model(
    ...
    # 10. Keep WAV-cloning dependencies beside the converted model.  Runtime
    # inference remains strictly offline and never repairs a partial install.
    print("\n[5/5] Installing pinned WAV reference preprocessing assets...")
    preprocessing_paths = install_wav_preprocessing_artifacts(output_dir)
    for path in preprocessing_paths:
        print(f"  Installed {path.relative_to(output_dir)}")
