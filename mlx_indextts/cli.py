...
def require_mlx_runtime(platform_name: str | None = None) -> None:
    current = sys.platform if platform_name is None else platform_name
    if current == "win32":
        raise SystemExit(
            "MLX 当前不支持 Windows 原生运行。请在 WSL 2 中执行此命令，"
            "或在 Voice Studio 源码目录运行 .\\start.ps1。"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="mlx-indextts",
        description="IndexTTS using MLX on macOS or Linux/WSL (supports v1.5 and v2.0)",
    )
